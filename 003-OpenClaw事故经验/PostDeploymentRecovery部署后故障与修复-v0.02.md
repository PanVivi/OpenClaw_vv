# 003 OpenClaw事故经验｜PostDeploymentRecovery 部署后故障与修复｜v0.02
...
| 2026-07-23 | Telegram 增量绑定反复失败，步非煙与魚玄機 polling 卡住 | `ops-telegram-admin` 连续多次写配置、即时 probe 失败即整份回滚，放大 OpenClaw channel stop 超时 | 插件已停用；改用官方 `channels add` 与 `agents bind` |
...
3. Telegram 原三条 account/binding 没有丢失；随后 coder / 步非煙已通过真实 Token 新增，当前共四条，reviewer 与三位 companion 共四条仍未创建。
...
前三条是事故前既有路由；coder、reviewer 与三位 companion 在后续增量建立。八条 binding 与八个有效 Bot probe 均存在；连续热重载后 coder 与 companion-dugu 出现 `channel stop timed out`，等待完整 Gateway 重启复验。

**修复**

- 恢复 Gateway 和运行时后，对既有 provider 做连接、实际出站和实际入站测试；
- 保留原 account ID、binding 和会话 ID；
- 不用虚构 token 创建空绑定；
- 完成配置后停止继续写入，只做一次完整 Gateway 重启，并逐个验收八 Bot polling 与真实收发。
...
### 10. 专用 Telegram 绑定插件放大热重载故障

**现象**

- 魚玄機调用专用插件绑定 coder 或 companion 时经常报告运行 probe 失败；
- account 可能已经落盘并启动，插件仍按失败恢复整份配置；
- coder 与 default/魚玄機随后出现 `channel stop exceeded 5000ms`、polling 卡住。

**根因**

- 插件把账号新增、策略修改、binding、配置校验和即时运行 probe 绑定为一个事务；
- 一次操作至少多次改写配置并触发 provider 热重载；
- 短时 probe 未就绪被错误提升为配置失败，整份回滚再次触发热重载；
- OpenClaw 的 channel stop 超时是底层问题，插件是直接触发器和放大器。

**修复**

1. 禁用 `ops-telegram-admin`，从 ops allowlist 移除工具；
2. 保留插件文件、四个 account/binding、Token、session、transcript 和 memory；
3. 保留魚玄機受审 Gateway `exec/process`；
4. 后续使用官方 `channels add` 与 `agents bind`；
5. 配置结果与运行态结果分开判断，不因短时 probe 未就绪回滚整份配置；
6. 完成全部写入后必要时只重启一次 Gateway，再验收 polling、probe 与真实收发。

插件停用修复完成时，四个既有 account 均 `running=true`、`connected=true`、`probe.ok=true`、`restartPending=false`，session 相关文件数量为 `567`。18:09 后另一活动会话继续新增四个账号并修正凭据；截至 18:20，八个 probe 均通过，六个 connected 正常，coder 与 companion-dugu 因 channel stop 超时等待完整重启。session 相关文件为 `567`、其中 JSONL `267`。

## 四、已验证恢复状态

截至 2026-07-23 18:20 +08:00：
...
- 八条 Telegram binding 与有效 probe 存在；六个 account 运行正常，coder 与 companion-dugu 因 channel stop 超时处于 `restartPending`；
- coder Sandbox 容器运行；
...
1. 需要一次完整 Gateway 重启清理 coder 与 companion-dugu 的 `restartPending`，随后补齐八 Bot 真实收发；
2. 完整自动长期记忆系统仍未部署；
