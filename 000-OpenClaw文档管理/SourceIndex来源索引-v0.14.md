# 000 OpenClaw文档管理｜SourceIndex 来源索引｜v0.14
...
- 当前进度：`002-OpenClaw部署进度/CurrentProgress当前进度-v0.15.md`
...
- Housekeeper Async Dispatch 报告：`002-OpenClaw部署进度/HousekeeperAsyncDispatch贾南风非阻塞委派报告-v0.01.md`
- Ops Telegram Admin 报告：`002-OpenClaw部署进度/OpsTelegramAdmin鱼玄机Telegram运维插件部署报告-v0.01.md`
...
賈南風 v1.10、蕭觀音 v0.07、魚玄機 v0.09、步非煙 v0.07、夏姬 / reviewer v0.05、三位 companion v0.04。

## 当前部署原则

賈南風负责接单、拆分、异步委派和结果汇总。长任务在独立 session key 中执行，Telegram 前台不再同步等待执行 Agent；同一 session 仍保持单写者一致性。

`life-automation` 仅向 life 暴露受限生活自动化工具。`ops-telegram-admin` 仅向 ops 暴露五个固定 Agent 的 Telegram 增量账号与 binding 工具。两者都不依赖 Codex 或管理员在线，也不授予通用宿主执行。
