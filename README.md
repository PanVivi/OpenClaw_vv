...
2. [快速简报 v1.08](001-OpenClaw规划设计/QuickBrief快速简报-v1.08.md)
...
11. [实施路线 v0.10](001-OpenClaw规划设计/ImplementationRoadmap实施路线图-v0.10.md)
12. [当前进度 v0.18](002-OpenClaw部署进度/CurrentProgress当前进度-v0.18.md)
...
19. [来源索引 v0.17](000-OpenClaw文档管理/SourceIndex来源索引-v0.17.md)
...
八 Agent workspace、基础五文件、八个 Telegram account 和八条 account-scoped binding 均存在。当前 default、housekeeper、life、coder、reviewer、companion-dugu、companion-lv 七条 connected/probe 正常；companion-wu 的 Token probe 为 HTTP 404，需取得有效 Token 后修复。不得改动无关 routing、transcript、session 或个人记忆。
...
魚玄機在少主直接任务或 housekeeper 正式委派形成的任务级授权包内，使用经 `mode=auto` 和精确参数白名单约束的 NAS Gateway `exec/process` 执行工程配置、服务和部署；授权包内不逐步骤索权，只有严重例外暂停上报。Telegram account/binding 使用 OpenClaw 原生 `channels add` 与 `agents bind`，`ops-telegram-admin` 已停用。任意 Gateway、Cron、history、spawn 和任意外发仍关闭。
