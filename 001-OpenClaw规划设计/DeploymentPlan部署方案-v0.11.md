# 001 OpenClaw架设部署｜DeploymentPlan 部署方案｜v0.11
...
1. 先做全量可恢复备份并记录会话、transcript、记忆和 Bot 基线。
2. 启用官方 Workboard，验证插件加载、配置和 Gateway。
3. 部署窄适配插件；housekeeper 只获得控制面工具，worker 只获得本卡 claim/heartbeat/proof/complete/block。
4. 增量部署八角色五件套；人格、身份和原权限不重写。
5. 建立 `production` 工作板、终态订阅和 Telegram 通知泵。
6. 建立固定命令调度泵；由它推进父子依赖、回收/阻塞超时卡并启动 ready 卡。
7. 迁移旧看门狗终态后停用旧插件；旧文件只读保留。
8. 依次验证成功、阻塞、失联、条件续办、并发、重启恢复、通知、A2A、Telegram 和负向权限。
9. 运行 `tasks audit`、配置校验、插件诊断、8 Bot probe、数据计数和秘密扫描。
10. 只在全部通过后更新文档并同步 Git。

回滚优先级：停用两个 Workboard Cron → 重新启用旧插件配置与工具 → 从
`backups/workboard-task-control-20260724T191932` 恢复配置/角色文件。不得覆盖或删除现有会话、transcript、记忆和 Telegram 凭据。
