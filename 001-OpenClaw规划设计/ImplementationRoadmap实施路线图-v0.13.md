# 001 OpenClaw架设部署｜ImplementationRoadmap 实施路线图｜v0.13

本路线图以 v0.04 完整路线为不可删减底稿，按 FinalDesign v1.08、共同协议 v0.05 和当前运行状态增量更新。

## 1. 当前状态

```text
已完成：八 Agent workspace、角色五件套、Telegram account/binding、A2A、记忆隔离
本轮：恢复完整文档基线、统一风险分级、部署全员同角色非阻塞子 Agent
固定结构：housekeeper / ops / coder / reviewer / life / companion-dugu / companion-wu / companion-lv
```
...
1. **已完成**：八角色 workspace、角色五件套、Telegram 账号与 binding、A2A 主动双向、个人记忆隔离。
2. **已完成**：life 持久自动化、housekeeper 跨角色异步派发、ops 任务级授权与原生 Telegram 绑定流程。
3. **本轮完成目标**：恢复完整总设计底稿；统一三档风险；把同角色隔离子 Agent部署到八个主 Agent；保持生产工具边界不变。
4. **本轮验收**：配置 validate、八角色 spawn Smoke Test、Telegram probe、Gateway 健康、A2A/路由/记忆不回归、文档继承矩阵。
5. **后续仅按需求**：真实长期任务压测、性能参数微调。不得把新的管理面、额外 Agent、全局历史读取或人格改写加入本轮。

版本管理要求：当前快照先归档后升版；能从固定 Git 提交恢复的缺档按原文恢复，不能精确恢复的只记录缺口，不伪造历史版本。

## v0.12 已完成里程碑：Workboard 任务控制

1. 官方文档、OpenClaw GitHub issue 与社群材料交叉核对，确定不引入第三方 Mission Control。
2. 备份配置、角色卡、会话、transcript、记忆、旧看门狗状态并校验哈希。
3. 启用官方 Workboard，建立 `production` 工作板。
4. 部署 `housekeeper-workboard-control 1.0.3` 窄适配层和八角色 Workboard 权限。
5. 建立每分钟 `WorkboardDispatchPump` 与每两分钟 `WorkboardNotificationPump`。
6. 将旧看门狗两条终态迁为只读历史卡，停用旧插件和工具入口，不删除旧状态。
7. 完成成功、受控阻塞、超时失联、并发、条件续办、重启恢复、A2A、Telegram、权限负向和数据无损验收。

后续路线只包含实际使用观察和按证据修正；不得重新引入聊天轮询式看门狗，也不得把一般任务执行重新塞回主 Agent 会话。

## Codex 专属任务面板阶段

1. 建立独立 `codex` board，并保持生产 dispatcher 固定 `--board production`。
2. 賈南風 v1.15 增量接入登记、查询、取消与最终通知，不新增工程权限。
3. 部署仓库外 DPAPI 凭据和确定性 Windows 客户端。
4. 建立 Codex Desktop 每小时 standalone Scheduled；空队列轻扫，发现卡后由本次独立 run 直接执行，不嵌套第二层 task。
5. 以低副作用卡真实验证定时触发、官方 claim、run 映射、三审、结果回写、去重和 Telegram。
6. 后续只观察自然小时运行、失败恢复和额度消耗；不得把空扫描改成 OpenClaw 模型轮询。
