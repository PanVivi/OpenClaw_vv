...
const ParamsSchema = Type.Object({
    ...
    agent_id: Type.Optional(Type.Union([
        Type.Literal("coder"),
    ...
    bot_token: Type.Optional(Type.String({
        minLength: 25,
        maxLength: 200,
...
async function getBindings() {
    return parseJsonOutput(await runCli(["agents", "bindings", "--json"]));
}
async function getAccounts() {
    return parseJsonOutput(await runCli(["config", "get", "channels.telegram.accounts"]));
}
async function getChannelStatus() {
    return parseJsonOutput(await runCli(["channels", "status", "--probe", "--json"]));
}
function accountStatus(status, accountId) {
    const channelAccounts = status.channelAccounts;
    const telegram = channelAccounts?.telegram;
    if (!Array.isArray(telegram))
        return undefined;
    return telegram.find((entry) => typeof entry === "object" &&
        entry !== null &&
...
function botIdFromAccount(entry) {
...
export default definePluginEntry({
    ...
    description: "Constrained Telegram account and binding administration for the ops agent.",
    register(api) {
        ...
        api.registerTool((ctx) => {
            ...
            return {
                ...
                async execute(_toolCallId, rawParams) {
                    ...
                    try {
                        if (!ownerUserId)
                            throw new Error("Plugin config ownerUserId is required");
                        if (params.action === "status") {
                            ...
                            const targets = Object.keys(TARGETS).map((agentId) => {
                                ...
                                return {
                                    agentId,
                                    displayName: TARGETS[agentId],
                                    accountConfigured: typeof account === "object" && account !== null,
                                    bindingConfigured: Boolean(binding),
                                    running: runtime?.running === true,
                                    connected: runtime?.connected === true,
                                    probeOk: runtime?.probe?.ok === true,
                                    lastError: typeof runtime?.lastError === "string"
                                        ? runtime.lastError
                                        : null
                                };
                            });
                        ...
                        return await withLock(async () => {
                            const [accountsBefore, bindingsBefore, statusBefore, telegramBot] = await Promise.all([
                                getAccounts(),
                                getBindings(),
                                getChannelStatus(),
                            ...
                            if (typeof accountsBefore[target] === "object") {
                                throw new Error(`Telegram account ${target} already exists; replacement is intentionally unsupported`);
                            }
                            const conflictingBinding = bindingsBefore.find((entry) => entry.match?.channel === "telegram" &&
                                (entry.agentId === target || entry.match?.accountId === target));
                            if (conflictingBinding) {
                                throw new Error(`A conflicting Telegram binding already exists for ${target}`);
                            }
                            ...
                            if (duplicate) {
                                throw new Error(`This Bot Token is already used by Telegram account ${duplicate.accountId}`);
                            }
                            ...
                            const backupDir = join(home, ".openclaw", "backups", `ops-telegram-admin-${stamp}`);
                            ...
                            try {
                                ...
                                if (runtime?.running !== true ||
                                    ...
                                    throw new Error(`Telegram account ${target} did not pass runtime probe`);
                                }
                                return result({
                                    ok: true,
                                    configured: true,
                        ...
                        const message = sanitize(error instanceof Error ? error.message : String(error), token);
