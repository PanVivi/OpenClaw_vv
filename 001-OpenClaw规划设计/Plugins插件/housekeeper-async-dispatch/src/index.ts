...
type WatchStatus = "waiting" | "progress" | "completed" | "blocked" | "cancelled";
type WatchRecord = {
  taskId: string;
  target: string;
...
type WatchState = { version: 1; tasks: WatchRecord[] };

const TOOL_NAME = "housekeeper_task_watch";
const TICK_MS = 15_000;
const TASK_ID_PATTERN =
  /(?:Task\s*ID|任务\s*ID|任務\s*ID)[*_` \t]*[:：=][*_` \t]*([A-Za-z0-9][A-Za-z0-9._-]{2,99})/i;
...
const ParamsSchema = Type.Object(
  {
    ...
    next_report_minutes: Type.Optional(
      Type.Integer({ minimum: 2, maximum: 1440 })
    )
  },
...
function taskIdFromMessage(value: unknown): string | undefined {
  if (typeof value !== "string") return undefined;
  return TASK_ID_PATTERN.exec(value)?.[1];
}

function targetFromParams(params: JsonRecord): string {
...
export function buildStallAlertMessage(record: WatchRecord): string {
  return [
...
export default definePluginEntry({
  ...
  description:
    "Keep housekeeper delegation responsive and report stalled A2A work.",
  register(api) {
    ...
    const stallMinutes =
      Number.isInteger(configuredStallMinutes) &&
    ...
    const readState = async (): Promise<WatchState> => {
      try {
        const value = JSON.parse(await readFile(statePath, "utf8")) as WatchState;
    ...
    const writeState = async (state: WatchState) => {
    ...
    const withStateLock = async <T>(operation: () => Promise<T>): Promise<T> => {
      ...
      try {
        ...
        if (!acquired) throw new Error("Timed out acquiring housekeeper task state lock");
        try {
    ...
    const upsertDispatch = async (
      ...
      await withStateLock(async () => {
        ...
        const record: WatchRecord = existing ?? {
          taskId,
          target,
          sourceSessionKey,
          status: "waiting",
        ...
        record.dueAt = new Date(
          now.getTime() + stallMinutes * 60_000
        ).toISOString();
        record.note = "A2A delegation accepted; waiting for acknowledgement or progress";
        if (!existing) state.tasks.push(record);
        await writeState(state);
      });
    };

    const updateFromInterSession = async (prompt: string) => {
      if (
      ...
      await withStateLock(async () => {
        ...
        const explicitStatus =
          /(?:Status|状态|狀態)\s*[:：]\s*(completed|complete|done|failed|error|blocked|cancelled|canceled)/i.exec(
            prompt
          )?.[1]?.toLowerCase();
        let status: WatchStatus = "progress";
        if (
    ...
    const alertStalledTask = async (record: WatchRecord) => {
      if (!ownerSessionKey) {
      ...
      try {
        const due = await withStateLock(async () => {
          ...
          const selected = state.tasks
            .filter(
              (task) =>
                (task.status === "waiting" || task.status === "progress") &&
        ...
        api.logger.error(
          `housekeeper watchdog tick failed: ${
        ...
        if (taskId) {
          try {
            await upsertDispatch(
              taskId,
              targetFromParams(event.params),
              ctx.sessionKey ?? ownerSessionKey ?? "unknown"
            );
          } catch (error) {
            api.logger.error(
              `housekeeper task watch could not persist ${taskId}; delegation continues: ${
                error instanceof Error ? error.message : String(error)
              }`
            );
          }
          ...
          api.logger.warn(
            `housekeeper sessions_send has no Task ID and cannot be watched (session=${ctx.sessionKey ?? "unknown"})`
          );
        }
    ...
    api.on(
      "before_agent_start",
      async (event, ctx) => {
        if (ctx.agentId !== allowedAgentId) return;
        try {
          await updateFromInterSession(event.prompt);
        } catch (error) {
          api.logger.error(
            `housekeeper task watch could not process A2A update; agent run continues: ${
              error instanceof Error ? error.message : String(error)
            }`
          );
        }
    ...
    api.registerTool(
      (ctx) => {
        ...
        return {
          ...
          description:
            "List or update persistent housekeeper A2A task watches. Every target acknowledgement/progress/terminal report must update its Task ID. completed/cancelled closes the watch; blocked reports immediately and remains visible.",
          ...
          async execute(_toolCallId, rawParams) {
            ...
            try {
              return await withStateLock(async () => {
                ...
                const record = state.tasks.find((task) => task.taskId === taskId);
                if (!record) throw new Error(`Task watch not found: ${taskId}`);
                ...
                const now = new Date();
                record.status = params.status as WatchStatus;
                record.lastUpdateAt = now.toISOString();
                record.note =
