...
export function buildDispatchArgs(params: JsonRecord): string[] {
...
export function buildShowArgs(params: JsonRecord): string[] {
  return ["workboard", "show", String(params.card_id), "--json"];
}

export function resolveCliInvocation(): { command: string; prefix: string[] } {
...
const plugin: ReturnType<typeof definePluginEntry> = definePluginEntry({
  ...
  description:
    "Trigger the official Workboard dispatcher without granting housekeeper general exec.",
  register(api) {
    ...
    const executeCli = async (args: string[]) => {
      try {
        ...
        const result = await api.runtime.system.runCommandWithTimeout(
          [invocation.command, ...invocation.prefix, ...args],
          { timeoutMs }
        );
    ...
    api.registerTool(
      (ctx) => {
        ...
        return {
          ...
          description:
            "Start ready Workboard cards through the official Gateway dispatcher. Create/specify/decompose cards first. This tool owns no task state and cannot run arbitrary commands.",
          parameters: ParamsSchema,
          async execute(_toolCallId, rawParams) {
            return executeCli(buildDispatchArgs(rawParams as JsonRecord));
          }
        };
      },
    ...
    api.registerTool(
      (ctx) => {
        ...
        return {
          ...
          description:
            "Read one Workboard card by UUID through the official CLI. This is read-only and exposes no arbitrary command, path, or file access.",
          parameters: ShowParamsSchema,
          async execute(_toolCallId, rawParams) {
            return executeCli(buildShowArgs(rawParams as JsonRecord));
          }
        };
      },
