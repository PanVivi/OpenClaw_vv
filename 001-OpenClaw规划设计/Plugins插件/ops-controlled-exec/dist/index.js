...
export default definePluginEntry({
    ...
    register(api) {
        ...
        if (allowedAgentId !== "ops")
            throw new Error("ops-controlled-exec agentId must be ops");
        ...
        const configuredRoots = Array.isArray(cfg.allowedRoots) ? cfg.allowedRoots.filter((v) => typeof v === "string") : [stateDir];
        ...
        async function safePath(input) {
            ...
            if (!roots.some((root) => within(root, absolute)))
                throw new Error("path is outside configured read-only roots");
            const target = await realpath(absolute);
            if (!roots.some((root) => within(root, target)))
                throw new Error("resolved path escaped configured roots");
            ...
            return {
                ...
                async execute(_id, raw) {
                    ...
                    try {
                        ...
                        if (typeof p.action === "string" && commandMap[p.action])
                            return result({ ok: true, action: p.action, ...(await run(commandMap[p.action])) });
                        if (p.action === "cron_get")
                            return result({ ok: true, action: p.action, ...(await run(["cron", "get", required(p.id, "id"), "--json"])) });
                        if (p.action === "task_get")
                            return result({ ok: true, action: p.action, ...(await run(["tasks", "get", required(p.id, "id"), "--json"])) });
