...
type SourceInfo = {
  kind: "direct_life" | "housekeeper_handoff";
  sourceAt: string;
  handoffId?: string;
};

type ReminderState = {
  status: "scheduled" | "sending" | "sent" | "unknown" | "cancelled" | "expired";
...
type TaskRecord = {
  id: string;
  title: string;
  status: "todo" | "doing" | "done" | "failed" | "blocked" | "archived";
  dueDate?: string;
  note?: string;
  source: SourceInfo;
  dedupeKey: string;
...
type InputState = {
  ...
  revision: number;
  updatedAt: string;
  preferences: Partial<Record<ModuleName, Record<string, string | number | boolean>>>;
  events: EventRecord[];
  tasks: TaskRecord[];
  notes: NoteRecord[];
  locationOverrides: LocationRecord[];
};

type HandoffRecord = {
  id: string;
  status: "pending" | "applying" | "applied" | "blocked" | "cancelled";
...
type HandoffState = {
  schema: "hehuan.morning-brief-handoffs";
  schemaVersion: 1;
...
const CONTROL_ACTIONS = new Set<string>(CONTROL_ACTION_VALUES);
...
const HANDOFF_ALLOWED_ACTIONS = new Set<string>(HANDOFF_CONTROL_ACTION_VALUES);
const MODULE_VALUES = [
  "header", "weather", "aqi", "attire", "tasks", "disciples", "schedule", "folk_calendar"
] as const;
const PREFERENCE_KEYS: Record<ModuleName, Set<string>> = {
...
function moduleName(value: unknown): ModuleName {
  const raw = text(value, "module", 32) as ModuleName;
  if (!MODULES.has(raw)) throw new Error(`unsupported morning-brief module: ${raw}`);
  return raw;
}

function dedupe(value: unknown, fallback: string): string {
  if (typeof value === "string" && value.trim()) return text(value, "dedupe_key", 200);
  return createHash("sha256").update(fallback, "utf8").digest("hex").slice(0, 32);
}

function sourceInfo(params: JsonRecord, kind: SourceInfo["kind"], handoffId?: string): SourceInfo {
...
function emptyHandoffs(): HandoffState {
  return { schema: "hehuan.morning-brief-handoffs", schemaVersion: 1, handoffs: [] };
}

function validateInputs(value: unknown): InputState {
  const state = value as InputState;
  if (state?.schema !== "hehuan.morning-brief-inputs" || state.schemaVersion !== 1) {
    throw new Error("morning-brief input schema is invalid");
  }
  for (const key of ["events", "tasks", "notes", "locationOverrides"] as const) {
    if (!Array.isArray(state[key])) throw new Error(`morning-brief ${key} is invalid`);
  }
...
function validateHandoffs(value: unknown): HandoffState {
...
async function readJson<T>(path: string, fallback: T, validate: (value: unknown) => T): Promise<T> {
  try {
    const info = await stat(path);
    if (!info.isFile()) throw new Error("state target is not a regular file");
    if (info.size > 1024 * 1024) throw new Error("state file is too large");
...
function occurrenceAfter(event: EventRecord, afterMs: number): { occurrenceAt: string; reminderAt: string } | undefined {
  ...
  if (step === 0 && occurrence < afterMs) return undefined;
  return {
...
function scheduleReminder(event: EventRecord, afterMs = Date.now()) {
...
function dateInRange(target: string, start: string, end: string) {
...
function eventOccursOn(event: EventRecord, targetDate: string, timezone: string): boolean {
  if (event.status === "cancelled") return false;
  const baseDate = dateInTimezone(event.eventAt, timezone);
  if (event.recurrence === "none") return baseDate === targetDate;
...
function dayProjection(state: InputState, targetDate: string, timezone: string) {
  return {
    date: targetDate,
...
export default definePluginEntry({
  ...
  register(api) {
    const config = (api.pluginConfig ?? {}) as JsonRecord;
    ...
    for (const [name, candidate] of [["ownerRoot", ownerRoot], ["stateRoot", stateRoot], ["runtimeStatusPath", runtimeStatusPath]] as const) {
    ...
    const withLock = async <T>(operation: () => Promise<T>): Promise<T> => {
      ...
      stateQueue = new Promise<void>((resolveQueue) => { release = resolveQueue; });
      await previous;
    ...
    const saveInputs = async (state: InputState) => {
      state.revision += 1;
      state.updatedAt = new Date().toISOString();
      await atomicWrite(inputsPath, state);
    };
    const saveHandoffs = (state: HandoffState) => atomicWrite(handoffsPath, state);

    const resolveLocation = async (params: JsonRecord) => {
      ...
      if (!response.ok) throw new Error("地点查询暂时不可用");
      const body = await response.json() as { results?: Array<JsonRecord> };
      const candidates = (body.results ?? []).filter((item) => typeof item.latitude === "number" && typeof item.longitude === "number");
      if (candidates.length === 0) throw new Error("没有找到可核实的地点");
      ...
      if (!selected) throw new Error("地点候选编号无效");
      return {
    ...
    const applyControlAction = async (
      ...
      if (action === "upsert_preference") action = "set_preference";
      ...
      let resolvedLocation: JsonRecord | undefined;
      ...
      return withLock(async () => {
        ...
        if (action === "upsert_event") {
          ...
          const recurrence = (params.recurrence ?? "none") as EventRecord["recurrence"];
          if (!["none", "daily", "weekly"].includes(recurrence)) throw new Error("recurrence must be none, daily, or weekly");
          const status = (params.event_status ?? "confirmed") as EventRecord["status"];
          if (!["confirmed", "tentative"].includes(status)) throw new Error("event_status must be confirmed or tentative");
          ...
          const event: EventRecord = existing ?? {
        ...
        if (action === "cancel_event") {
          ...
          if (!event) throw new Error("未找到这项日程");
        ...
        if (action === "upsert_task") {
          ...
          const status = (params.task_status ?? "todo") as TaskRecord["status"];
          if (!["todo", "doing", "done", "failed", "blocked"].includes(status)) throw new Error("unsupported task_status");
        ...
        if (action === "set_task_status" || action === "archive_task") {
          ...
          if (!task) throw new Error("未找到这项待办");
          const status = action === "archive_task" ? "archived" : text(params.task_status, "task_status", 32) as TaskRecord["status"];
          if (!["todo", "doing", "done", "failed", "blocked", "archived"].includes(status)) throw new Error("unsupported task_status");
          task.status = status;
          task.source = source;
          task.updatedAt = now;
          await saveInputs(state);
        ...
        if (action === "upsert_note") {
          ...
          if (endDate < startDate) throw new Error("end_date must not be before start_date");
          ...
          const note: NoteRecord = existing ?? {
        ...
        if (action === "archive_note") {
          ...
          if (!note) throw new Error("未找到这条模块补记");
        ...
        if (action === "set_preference") {
          ...
          if (!PREFERENCE_KEYS[module].has(key)) throw new Error(`preference ${key} is not allowed for ${module}`);
          const value = params.preference_value;
          if (!["string", "number", "boolean"].includes(typeof value)) throw new Error("preference_value must be string, number, or boolean");
          state.preferences[module] ??= {};
          state.preferences[module]![key] = value as string | number | boolean;
          await saveInputs(state);
        ...
        if (action === "clear_preference") {
          ...
          if (changed) {
            ...
            if (Object.keys(state.preferences[module]!).length === 0) delete state.preferences[module];
            await saveInputs(state);
          }
        ...
        if (action === "set_location") {
          ...
          const record: LocationRecord = existing ?? {
          ...
          if (!existing) state.locationOverrides.push(record);
          await saveInputs(state);
        ...
        if (action === "clear_location") {
          ...
          if (!record) throw new Error("未找到这项地点覆盖");
    ...
    const applyHandoff = async (handoffId: string): Promise<JsonRecord> => {
      ...
      if (record.status === "applied") return { ok: true, applied: true, handoffId, result: record.resultSummary };
      try {
    ...
    const processDueReminders = async () => {
      if (ticking || !lifeSessionKey) return;
      ticking = true;
      try {
        const due = await withLock(async () => {
          ...
          for (const event of state.events) {
            if (event.status === "cancelled" || event.reminder.status !== "scheduled" || !event.reminder.nextRunAt) continue;
            if (Date.parse(event.reminder.nextRunAt) > now) continue;
          ...
          if (selected.length) await saveInputs(state);
          return selected;
        });
        for (const event of due) {
          ...
          try {
            ...
            if (waited.status !== "ok") errorText = waited.error ?? `提醒投递状态为 ${waited.status}`;
          } catch (error) {
          ...
          await withLock(async () => {
            ...
            if (!current || current.reminder.status !== "sending") return;
    ...
    api.registerTool((ctx) => {
      if (ctx.agentId !== lifeAgentId) return null;
      return {
        name: CONTROL_TOOL,
        label: "晨间玉简控制",
        ...
        async execute(_toolCallId, rawParams) {
          ...
          try {
            ...
            if (!CONTROL_ACTIONS.has(action)) throw new Error(`unsupported action: ${action}; use guide and one of: ${CONTROL_ACTION_VALUES.join(", ")}`);
            if (action === "guide") {
              return toolResult({
                ok: true,
                actions: {
            ...
            if (action === "inspect") {
              const [inputs, handoffs, runtime] = await Promise.all([
                readInputs(), readHandoffs(), readJson(runtimeStatusPath, {}, (value) => value as JsonRecord)
              ]);
    ...
    api.registerTool((ctx) => {
      if (ctx.agentId !== housekeeperAgentId) return null;
      return {
        name: HANDOFF_TOOL,
        label: "晨间玉简转交",
        ...
        async execute(_toolCallId, rawParams) {
          ...
          try {
            ...
            if (action === "status") {
              ...
              if (!record) throw new Error("未找到这项晨报转交");
            ...
            if (action === "cancel") {
              ...
              const record = await withLock(async () => {
                ...
                if (!found) throw new Error("未找到这项晨报转交");
                if (found.status === "applied") throw new Error("这项信息已经录入，需由蕭觀音修改或取消原记录");
            ...
            if (action !== "submit") throw new Error("action must be submit, status, or cancel");
            if (!lifeSessionKey) throw new Error("晨报转交尚未配置固定的蕭觀音会话");
            const controlAction = text(params.control_action, "control_action", 64);
            if (!HANDOFF_ALLOWED_ACTIONS.has(controlAction)) throw new Error(`不允许转交该操作: ${controlAction}`);
            const sourceAt = isoInstant(params.source_at, "source_at");
            const expiresAt = isoInstant(params.expires_at, "expires_at");
            if (Date.parse(expiresAt) <= Date.now()) throw new Error("转交有效期已经结束");
            const payload = params.payload && typeof params.payload === "object" && !Array.isArray(params.payload) ? params.payload as JsonRecord : {};
            ...
            if (record.status === "applied") return toolResult({ ok: true, applied: true, handoffId: record.id, result: record.resultSummary });
            if (record.status === "cancelled") return toolResult({ ok: false, applied: false, handoffId: record.id, error: "这项转交已取消" });
            const started = await api.runtime.subagent.run({
              sessionKey: lifeSessionKey,
