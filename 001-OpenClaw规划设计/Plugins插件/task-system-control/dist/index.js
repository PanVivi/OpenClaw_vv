...
const INTAKE_ACTIONS = ["triage", "start", "inspect", "list", "reconcile", "queue_notification", "block"];
const HANDOFF_ACTIONS = ["submit", "accept", "status", "cancel"];
const MODULE_ACTIONS = ["catalog", "expect", "inspect"];
const MODULE_LABELS = {
...
function toolResult(value) {
    ...
    return { content: [{ type: "text", text: JSON.stringify(guidance, null, 2) }], details: guidance };
}
export function classifyWorkflow(prompt) {
    ...
    if (/(请|帮我|查查|检查|核对|搜索|整理|列出|完成|通知|给出方案|处理)/u.test(text))
        return "simple_task";
    return "conversation";
}
export function inferModules(toolName, params, result) {
    if (toolName !== "morning_brief_control")
        return [];
    const explicit = typeof params.module === "string" && MODULE_VALUES.includes(params.module)
        ? [params.module] : [];
...
function validateState(value) {
    const state = value;
    if (state?.schema !== "hehuan.task-system-control" || state.schemaVersion !== 1)
        throw new Error("task-system state schema is invalid");
    for (const key of ["inbox", "handoffs", "links", "outbox"]) {
        if (!Array.isArray(state[key]))
            throw new Error(`task-system ${key} is invalid`);
    }
    return state;
}
async function readStateFile(path) {
    try {
        ...
        if (!info.isFile())
            throw new Error("task-system state target is not a regular file");
        if (info.size > 4 * 1024 * 1024)
            throw new Error("task-system state is too large");
...
function eventKeyOf(event, ctx, prompt) {
    const providerId = [event.messageId, event.updateId, event.eventId, ctx.runId].find((value) => typeof value === "string" && value.trim());
...
function currentInbox(state, sessionKey, id) {
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        const configuredAgents = cfg.agents && typeof cfg.agents === "object" && !Array.isArray(cfg.agents) ? cfg.agents : {};
        ...
        const gateway = async (method, params) => {
            if (!api.runtime.gateway.isAvailable())
                throw new Error("Gateway 内部接口暂不可用，任务已保留等待恢复");
            return api.runtime.gateway.request(method, params, { timeoutMs: gatewayTimeoutMs });
        };
        const flowFor = (requesterOrigin) => api.runtime.tasks.flow.bindSession({
            sessionKey: controllerSessionKey,
            ...(requesterOrigin && typeof requesterOrigin === "object" ? { requesterOrigin: requesterOrigin } : {})
        });
        const findCardByIdempotency = async (key) => {
            const listed = await gateway("workboard.cards.list", { boardId });
        ...
        const updateFlowFromLink = async (link) => {
            if (!link.flowId)
                return;
            const flows = flowFor();
            ...
            if (!flow || flow.syncMode !== "managed")
                return;
            const prior = flow.stateJson && typeof flow.stateJson === "object" && !Array.isArray(flow.stateJson) ? flow.stateJson : {};
            const stateJson = {
                ...prior,
        ...
        const reconcileLink = async (linkId) => {
            ...
            let task;
            try {
                const runs = await gateway("workboard.cards.runs", { id: link.cardId });
                card = runs.card;
                ...
                const tasks = await gateway("tasks.list", { agentId: link.assignee, limit: 200 });
                const matches = (tasks.tasks ?? []).filter((item) => (runId && item.runId === runId) || (sessionKey && item.childSessionKey === sessionKey));
                if (matches.length > 1)
                    throw new Error("同一执行关联到多条任务账本，已停止自动收口");
                task = matches[0];
                const next = await withState((state) => {
                    ...
                    if (cardStatus === "done" || taskStatus === "succeeded")
                        current.status = current.proofReceipt ? "succeeded" : "blocked";
                    else if (["failed", "timed_out", "cancelled", "lost"].includes(taskStatus))
        ...
        api.on("before_agent_run", async (event, ctx) => {
            ...
            if (!expectedAccount || channel !== "telegram")
                return;
            if (event.accountId && event.accountId !== expectedAccount)
                return;
            if (event.senderIsOwner !== true || (ownerTelegramId && event.senderId !== ownerTelegramId))
                return;
            const sessionKey = typeof ctx.sessionKey === "string" ? ctx.sessionKey : "";
            if (!sessionKey)
                return { outcome: "block", reason: "missing_session", message: "这句话还没有取得可靠的会话位置，我先不冒险处理。请稍后重发一次，我会从原处继续。" };
            try {
                const prompt = String(event.prompt ?? "");
                const key = eventKeyOf(event, ctx, prompt);
                ...
                const turnMapKey = turnKey(ctx, event);
                if (turnMapKey)
                    turnInbox.set(turnMapKey, inbox.id);
            }
            catch (error) {
        ...
        api.on("agent_turn_prepare", async (_event, ctx) => {
            const id = turnInbox.get(turnKey(ctx));
            if (!id)
                return;
            const state = await readState();
        ...
        api.on("before_agent_finalize", async (event, ctx) => {
            const id = turnInbox.get(turnKey(ctx, event));
            if (!id)
                return;
            const state = await readState();
            const inbox = state.inbox.find((item) => item.id === id);
            ...
            if (/\b(?:CWD|UUID|Host|Card|Task)\b|approval card|审批卡片|sessionKey|runId|flowId/iu.test(finalText)) {
                return { action: "revise", reason: "internal_language_leak", retry: { instruction: "重写最终回复：保持角色口吻，把目标、结果、影响和下一步讲清楚，但删除所有工程字段、英文控制词和工业卡片语言。", idempotencyKey: `task-language:${inbox.id}`, maxAttempts: 1 } };
            }
            if (inbox.status === "registered") {
                return { action: "revise", reason: "task_not_triaged", retry: { instruction: `本轮任务尚未按持久入口分流。立即调用 task_intake triage，内部编号为 ${inbox.id}；按工具结果继续，不要向少主展示编号。`, idempotencyKey: `task-triage:${inbox.id}`, maxAttempts: 1 } };
            }
            if (inbox.profile === "direct_module" && inbox.expectedModules.some((item) => item.status !== "applied")) {
                return { action: "revise", reason: "module_receipt_missing", retry: { instruction: "晨报模块尚无成功应用收据。请使用对应专用工具完成写入并回读；若条件不足，清楚说明缺什么，不得说已经录入。", idempotencyKey: `task-module:${inbox.id}`, maxAttempts: 1 } };
            }
        });
        ...
        api.on("after_tool_call", async (event, ctx) => {
                ...
                await withState((state) => {
                    ...
                    if (inbox) {
                        ...
                        for (const module of modules) {
                            ...
                            if (expectation)
                                Object.assign(expectation, { status: "applied", toolCallId: event.toolCallId, receiptHash, updatedAt: now });
                            else
                                inbox.expectedModules.push({ module, action: String(event.params.action ?? "write"), status: "applied", toolName: event.toolName, toolCallId: event.toolCallId, receiptHash, updatedAt: now });
                        }
                        if (modules.length)
            ...
            if (["workboard_complete", "workboard_proof"].includes(event.toolName) && success) {
                const cardId = [event.params.card_id, event.params.cardId, event.params.id].find((value) => typeof value === "string");
                const receiptHash = sha256(JSON.stringify({ tool: event.toolName, params: event.params, result: resultDetails }));
                let linkId;
                await withState((state) => {
        ...
        api.on("subagent_ended", async (event) => {
            ...
            await withState((state) => {
                ...
                linkId = link.id;
                if (["failed", "blocked"].includes(link.status))
                    terminalLink = structuredClone(link);
            });
            if (terminalLink) {
                ...
                try {
                    ...
                    await withState((state) => {
                        ...
                        if (current)
                            current.error = `${current.error ?? "执行阻塞"}；Workboard 收口待恢复：${message}`;
                    });
                }
                const latest = (await readState()).links.find((item) => item.id === closed.id);
                if (latest)
                    await updateFlowFromLink(latest);
            }
            else if (linkId)
                await reconcileLink(linkId);
        });
        ...
        api.registerTool((ctx) => {
            const agentId = typeof ctx.agentId === "string" ? ctx.agentId : "";
            const sessionKey = typeof ctx.sessionKey === "string" ? ctx.sessionKey : "";
            if (!accountByAgent.has(agentId) || !sessionKey)
                return null;
            return {
                name: "task_intake",
                label: "任务总入口",
                    ...
                    try {
                        ...
                        if (action === "triage") {
                            const updated = await withState((state) => {
                                ...
                                const profile = typeof p.profile === "string" && PROFILE_VALUES.includes(p.profile) ? p.profile : inbox.profile;
                        ...
                        if (action === "queue_notification") {
                            ...
                            const queued = await withState((state) => {
                                ...
                                if (existing)
                                    return existing;
                                const now = new Date().toISOString();
                                const created = { id: `outbox-${randomUUID()}`, eventKey, sessionKey, contentHash: hash, status: "pending", createdAt: now, updatedAt: now };
                        ...
                        if (action === "reconcile") {
                            const state = await readState();
                            const inbox = currentInbox(state, sessionKey, p.inbox_id);
                            for (const link of state.links.filter((item) => item.inboxId === inbox.id))
                                await reconcileLink(link.id);
                        ...
                        const start = await withState((state) => {
                            ...
                            if (inbox.status === "completed")
                                return { inbox: structuredClone(inbox), existing: true };
                            if (inbox.profile === "direct_module" || inbox.profile === "conversation")
                                throw new Error("该交代不应建立正式工作卡，请按当前分流执行");
                            inbox.goal = goal;
                            inbox.status = "starting";
                            inbox.updatedAt = new Date().toISOString();
                            inbox.revision += 1;
                            return { inbox: structuredClone(inbox), existing: false };
                        });
                        if (start.existing)
                            return toolResult({ ok: true, idempotent: true, item: publicInbox(start.inbox) });
                        ...
                        if (!card) {
                            card = (await gateway("workboard.cards.create", {
                        ...
                        const link = await withState((state) => {
                            ...
                            if (existing)
                                return existing;
                            const now = new Date().toISOString();
                            const created = { id: `link-${randomUUID()}`, inboxId: start.inbox.id, flowId, cardId, boardId, assignee, status: "card_ready", createdAt: now, updatedAt: now };
                            state.links.push(created);
                            const inbox = state.inbox.find((item) => item.id === start.inbox.id);
                            if (!inbox.cardIds.includes(cardId))
                                inbox.cardIds.push(cardId);
                            inbox.status = "linked";
                            inbox.updatedAt = now;
                            inbox.revision += 1;
                        ...
                        try {
                            const dispatch = await gateway("workboard.cards.dispatch", { boardId });
                            const started = Array.isArray(dispatch.started) ? dispatch.started.find((item) => item.cardId === cardId) : undefined;
                            if (started) {
                                await withState((state) => {
                                    ...
                                    current.status = "running";
                                    current.updatedAt = current.lastActivityAt = new Date().toISOString();
                                });
                            }
                        }
                        catch (error) {
        ...
        api.registerTool((ctx) => {
            ...
            if (!accountByAgent.has(sourceAgentId) || !sourceSessionKey)
                return null;
            return {
                name: "task_handoff",
                label: "门下事务转交",
                    ...
                    try {
                        ...
                        if (action === "cancel") {
                            ...
                            const handoff = await withState((state) => {
                                ...
                                if (found.status === "applied")
                                    throw new Error("这项转交已经应用，需修改原记录而不是取消转交");
                        ...
                        if (action === "accept") {
                            ...
                            const handoff = await withState((state) => {
                                ...
                                if (!found)
                                    throw new Error("未找到这项转交");
                                if (found.targetAgentId !== sourceAgentId || found.targetSessionKey !== sourceSessionKey)
                                    throw new Error("这项转交不属于当前角色或会话");
                                if (found.status === "applied")
                                    return structuredClone(found);
                                if (["cancelled", "blocked"].includes(found.status))
                                    throw new Error("这项转交当前不能接收");
                        ...
                        if (action !== "submit")
                            throw new Error(`unsupported action: ${action}`);
                        if (sourceAgentId !== "housekeeper")
                            throw new Error("只有賈南風可以发起跨角色正式转交");
                        const targetAgentId = requiredText(p.target_agent, "target_agent", 80);
                        if (!accountByAgent.has(targetAgentId) || targetAgentId === "housekeeper")
                            throw new Error("目标角色不在受控转交目录中");
                        const targetSessionKey = typeof targetSessions[targetAgentId] === "string" ? String(targetSessions[targetAgentId]) : "";
                        if (!targetSessionKey)
                            throw new Error("目标角色尚未配置固定接收会话");
                        const capability = requiredText(p.capability, "capability", 160);
                        const payload = p.payload && typeof p.payload === "object" && !Array.isArray(p.payload) ? p.payload : {};
                        ...
                        if (handoff.status === "applied")
                            return toolResult({ ok: true, applied: true, status: handoff.status });
        ...
        api.registerTool((ctx) => {
            const sessionKey = typeof ctx.sessionKey === "string" ? ctx.sessionKey : "";
            if (!ctx.agentId || !accountByAgent.has(ctx.agentId) || !sessionKey)
                return null;
            return {
                name: "task_module",
                label: "晨间玉简模块登记",
                ...
                async execute(_id, raw) {
                    ...
                    try {
                        const action = requiredText(p.action, "action", 32);
                        if (action === "catalog")
                            return toolResult({ ok: true, modules: MODULE_VALUES.map((module) => ({ label: MODULE_LABELS[module], module, writeTool: "morning_brief_control" })), rule: "八个模块都由蕭觀音的同一权威数据源录入；賈南風只能走持久转交。" });
                        if (action === "inspect") {
                            const state = await readState();
                        ...
                        if (action !== "expect")
                            throw new Error(`unsupported action: ${action}`);
                        const module = requiredText(p.module, "module", 32);
                        if (!MODULE_VALUES.includes(module))
                            throw new Error("该栏目不在晨间玉简八模块目录中");
                        ...
                        const updated = await withState((state) => {
                            ...
                            if (existing) {
                                ...
                                inbox.expectedModules.push({ module, action: controlAction, status: "expected", toolName: "morning_brief_control", updatedAt: now });
                            inbox.profile = "direct_module";
                            inbox.status = "triaged";
                            inbox.updatedAt = now;
                            inbox.revision += 1;
                            return structuredClone(inbox);
                        });
