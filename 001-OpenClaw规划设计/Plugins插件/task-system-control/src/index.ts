...
import { definePluginEntry, type OpenClawConfig } from "openclaw/plugin-sdk/core";

type JsonRecord = Record<string, unknown>;
type WorkflowProfile = "conversation" | "direct_module" | "simple_task" | "governed_change" | "research_plan_triple_review";
type InboxStatus = "registered" | "triaged" | "delegating" | "reconcile_required" | "running" | "completed" | "blocked" | "cancelled";
type PacketStatus = "waiting_dependencies" | "pending" | "decomposing" | "reconcile_required" | "running" | "succeeded" | "blocked" | "cancelled";
type UnitStatus = "waiting_dependencies" | "ready" | "starting" | "reconcile_required" | "running" | "succeeded" | "failed" | "blocked" | "cancelled";
type DelegationMode = "direct_subagent_v3" | "legacy_controller_v2" | "legacy_workboard_v2";
...
type PlanRecord = {
  ...
  status: "planned" | "running" | "succeeded" | "blocked" | "cancelled";
  packetIds: string[];
  flowId?: string;
  completionWakeAt?: string;
  legacyCompletionWakeAt?: string;
  completionNotificationStatus?: "failed_before_dispatch" | "acquired" | "sent" | "unknown" | "quarantined";
  completionNotificationError?: string;
  createdAt: string;
  updatedAt: string;
  error?: string;
};
...
type PacketRecord = {
  ...
  controllerLeaseUntil?: string;
  decompositionReceipt?: string;
  workUnitIds: string[];
  createdAt: string;
  updatedAt: string;
  error?: string;
...
type LinkRecord = {
  ...
  boardId?: string;
  assignee: string;
  dispatchKind?: "subagent_direct" | "workboard_legacy";
  generation?: number;
  launchNonce?: string;
  idempotencyKey?: string;
  runId?: string;
  ...
  status: "card_ready" | "starting" | "running" | "succeeded" | "failed" | "blocked" | "cancelled" | "reconcile_required";
  proofReceipt?: string;
  completionReceipt?: string;
  proofSummary?: string;
  completionSummary?: string;
  workerOutcome?: string;
  observedEndedRunId?: string;
  observedEndedOutcome?: string;
  observedEndedAt?: number;
  observedEndedError?: string;
...
type LegacyHandoffRecord = JsonRecord & { id?: string; status?: string };
type OutboxRecord = {
  id: string;
  eventKey: string;
  kind: "ordinary_agent" | "completion_direct";
  sessionKey: string;
  contentHash: string;
  status: "pending" | "failed_before_dispatch" | "acquired" | "sent" | "unknown" | "failed" | "quarantined";
  planId?: string;
  attemptId?: string;
  generation?: number;
  accountId?: string;
...
type TaskSystemState = {
  ...
  links: LinkRecord[];
  handoffs: LegacyHandoffRecord[];
  outbox: OutboxRecord[];
};

type CapabilityRule = { owners: string[]; requiredTools: string[]; readOnly: boolean; sideEffect: boolean };
type TurnAssociation = { runKey: string; sessionKey: string; inboxId: string; contentHash: string; delegated: boolean; expiresAt: number };

const AGENT_IDS = ["housekeeper", "ops", "coder", "reviewer", "life", "companion-dugu", "companion-wu", "companion-lv"] as const;
...
const RECEIPT_TOOLS = ["task_work_proof", "task_work_complete"] as const;
...
const DEFAULT_CAPABILITIES: Record<string, CapabilityRule> = {
...
function legacyPlaceholderFingerprint(unit: WorkUnitRecord): string {
...
function isLegacyPlaceholder(unit: WorkUnitRecord): boolean {
...
function strings(value: unknown, field: string): string[] {
  if (!Array.isArray(value) || !value.length) throw new Error(`${field} is required`);
  return value.map((item) => text(item, field, 1000));
}
function record(value: unknown): JsonRecord { return value && typeof value === "object" && !Array.isArray(value) ? value as JsonRecord : {}; }
function detailsOf(value: unknown): JsonRecord { const r = record(value); return record(r.details ?? r); }
...
export function classifyWorkflow(prompt: string): WorkflowProfile {
  ...
  if (/(查询.*官方|官方说明|GitHub.*讨论|社群.*讨论|三次.*审查|完整.*收集|正式修复计划|修复整个|部署.*验收.*文档|全部结束.*通知)/isu.test(value)) return "research_plan_triple_review";
  if (/(生产|部署|配置|插件|权限|工作流|任务系统|修复|迁移|同步|重启|故障|回滚|验收)/u.test(value) && /(检查|核对|修复|实现|修改|部署|迁移|同步|解决|打通)/u.test(value)) return "governed_change";
  if (/(请|帮我|查查|调查|检查|核对|搜索|读取|整理|列出|完成|通知|给出方案|处理|录入)/u.test(value)) return "simple_task";
  return "conversation";
}

export function inferModules(toolName: string, params: JsonRecord, result: unknown): string[] {
  if (toolName !== "morning_brief_control") return [];
  if (typeof params.module === "string" && MODULE_VALUES.includes(params.module as typeof MODULE_VALUES[number])) return [params.module];
  ...
  const sections: unknown[] = Array.isArray(detailsOf(result).sections) ? detailsOf(result).sections as unknown[] : [];
...
export function migrateState(value: unknown): TaskSystemState {
  const raw = record(value);
  if (raw.schema !== "hehuan.task-system-control") throw new Error("task-system state schema is invalid");
  const base = emptyState();
  const inbox = Array.isArray(raw.inbox) ? raw.inbox.map((item) => {
    const r = record(item);
    return { ...r, prompt: typeof r.prompt === "string" ? r.prompt : "", planIds: Array.isArray(r.planIds) ? r.planIds : [], cardIds: Array.isArray(r.cardIds) ? r.cardIds : [], expectedModules: Array.isArray(r.expectedModules) ? r.expectedModules : [] } as InboxRecord;
  }) : [];
  const outbox = Array.isArray(raw.outbox) ? raw.outbox.map((item) => {
    const existing = record(item);
    if (existing.kind === "ordinary_agent" || existing.kind === "completion_direct") return existing as OutboxRecord;
    const eventKey = typeof existing.eventKey === "string" ? existing.eventKey : "";
    if (eventKey.startsWith("task-plan-completion:")) {
      return { ...existing, kind: "ordinary_agent", status: "quarantined", error: sanitizeLegacyError(existing.error, "旧消息记录占用了完成通知命名空间，已隔离") } as OutboxRecord;
    }
    return { ...existing, kind: "ordinary_agent" } as OutboxRecord;
  }) : [];
  const plans = Array.isArray(raw.plans) ? raw.plans.map((item) => {
    const existing = record(item);
    const plan = { ...existing, delegationMode: existing.delegationMode as DelegationMode | undefined } as PlanRecord;
    if (plan.completionWakeAt && !plan.completionNotificationStatus) {
      ...
      if (direct) plan.completionNotificationStatus = "sent";
      else {
  ...
  return {
    ...
    inbox,
    plans,
    packets: Array.isArray(raw.packets) ? raw.packets.map((item) => ({ ...record(item), inputRefs: Array.isArray(record(item).inputRefs) ? record(item).inputRefs : [], workUnitIds: Array.isArray(record(item).workUnitIds) ? record(item).workUnitIds : [] })) as PacketRecord[] : [],
    workUnits: Array.isArray(raw.workUnits) ? raw.workUnits.map((item) => ({ ...record(item), inputRefs: Array.isArray(record(item).inputRefs) ? record(item).inputRefs : [] })) as WorkUnitRecord[] : [],
    links: Array.isArray(raw.links) ? raw.links.map((item) => ({ ...record(item), dispatchKind: record(item).dispatchKind ?? (record(item).cardId ? "workboard_legacy" : undefined) })) as LinkRecord[] : [],
    handoffs: Array.isArray(raw.handoffs) ? raw.handoffs as LegacyHandoffRecord[] : [],
    outbox
  };
}

function sanitizeLegacyError(value: unknown, fallback: string): string {
...
async function readStateFile(path: string): Promise<TaskSystemState> {
  try {
    ...
    if (!info.isFile() || info.size > 16 * 1024 * 1024) throw new Error("task-system state target is invalid");
...
function providerOf(ctx: JsonRecord): string {
...
function sessionKind(sessionKey: string, controllers: Map<string, string>): "controller" | "worker" | "automation" | "main" {
  if ([...controllers.values()].includes(sessionKey)) return "controller";
  if (/:subagent:(?:workboard-|task-system-)/u.test(sessionKey)) return "worker";
  if (/:cron:|:automation:/u.test(sessionKey)) return "automation";
  return "main";
}

function assertAcyclic(keys: string[], dependencyMap: Map<string, string[]>, label: string) {
  const allowed = new Set(keys);
  const visiting = new Set<string>();
  const visited = new Set<string>();
  const visit = (key: string) => {
    if (visiting.has(key)) throw new Error(`${label} dependency graph contains a cycle`);
    if (visited.has(key)) return;
    visiting.add(key);
    for (const dep of dependencyMap.get(key) ?? []) {
      if (!allowed.has(dep)) throw new Error(`${label} dependency references an unknown key`);
      if (dep === key) throw new Error(`${label} cannot depend on itself`);
      visit(dep);
    }
...
function publicPlan(state: TaskSystemState, plan: PlanRecord) {
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    const accountByAgent = new Map<string, string>(AGENT_IDS.map((agentId) => [agentId, typeof accountConfig[agentId] === "string" ? String(accountConfig[agentId]) : agentId === "ops" ? "default" : agentId]));
    const controllerConfig = record(cfg.controllerSessions);
    const controllers = new Map<string, string>(AGENT_IDS.map((agentId) => [agentId, typeof controllerConfig[agentId] === "string" ? String(controllerConfig[agentId]) : `agent:${agentId}:task-controller`]));
    const capabilityConfig = record(cfg.capabilities);
    const capabilities = new Map<string, CapabilityRule>(Object.entries(DEFAULT_CAPABILITIES));
    ...
    const associations: TurnAssociation[] = [];
    ...
    const withState = async <T>(mutate: (state: TaskSystemState) => T | Promise<T>): Promise<T> => {
      const operation = mutationQueue.then(async () => {
        ...
        state.revision += 1; state.updatedAt = nowIso();
        state.inbox = state.inbox.slice(-5000); state.plans = state.plans.slice(-5000); state.packets = state.packets.slice(-10000);
        state.workUnits = state.workUnits.slice(-20000); state.links = state.links.slice(-20000); state.handoffs = state.handoffs.slice(-5000); state.outbox = state.outbox.slice(-5000);
    ...
    const terminal = (status: string) => ["succeeded", "completed", "failed", "blocked", "cancelled"].includes(status);
    const sanitizeSummary = (value: unknown, max = 1000) => String(value).normalize("NFKC").replace(/(token|secret|password|authorization)\s*[:=]\s*\S+/giu, "$1=[redacted]").replace(/[\u0000-\u001f\u007f]/gu, " ").trim().slice(0, max);
    const safeError = (error: unknown) => sanitizeSummary(error instanceof Error ? error.message : error, 1000);
    const receiptToolsAllowed = (agentId: string) => {
    ...
    const currentInbox = (state: TaskSystemState, sessionKey: string, id?: unknown) => {
    ...
    const capabilityRule = (name: string, owner: string) => {
      const rule = capabilities.get(name);
      if (!rule || !rule.owners.includes(owner)) throw new Error(`目标角色不具备声明能力 ${name}，请重新分工`);
      return rule;
    };

    const sendPlanCompletion = async (planId: string, allowPreflightRetry = false) => {
      const snapshot = await readState();
      const plan = snapshot.plans.find((item) => item.id === planId);
      if (!plan || !terminal(plan.status) || plan.completionNotificationStatus === "sent" || ["acquired", "unknown", "quarantined"].includes(plan.completionNotificationStatus ?? "")) return;
      ...
      let adapter: Awaited<ReturnType<typeof api.runtime.channel.outbound.loadAdapter>>;
      let preflightError = "";
      try {
        adapter = await api.runtime.channel.outbound.loadAdapter("telegram");
        if (!adapter?.sendText) throw new Error("Telegram 发送通道尚未就绪");
        if (!inbox || !ownerTelegramId || target !== ownerTelegramId) throw new Error("Telegram 收件人身份核对失败");
        if (!accountId) throw new Error("贾南风 Telegram 账号尚未配置");
      } catch (error) {
        ...
        await withState((state) => {
          ...
          if (!existing) state.outbox.push({
            ...
            existing.error = preflightError; existing.updatedAt = nowIso();
          }
      ...
      const claim = await withState((state) => {
        ...
        const existing = state.outbox.find((item) => item.eventKey === eventKey);
        if (existing && (existing.kind !== "completion_direct" || existing.planId !== planId)) return undefined;
        if (existing && !(allowPreflightRetry && existing.status === "failed_before_dispatch")) return undefined;
        const attemptId = randomUUID();
        const generation = existing ? (existing.generation ?? 0) + 1 : 1;
        const timestamp = nowIso();
        const item: OutboxRecord = existing ?? {
          id: `outbox-${randomUUID()}`, eventKey, kind: "completion_direct", planId, sessionKey: current.sourceSessionKey,
        ...
        if (!existing) state.outbox.push(item);
      ...
      if (!claim || !adapter?.sendText) return;

      try {
        const delivered = await adapter.sendText({ cfg: api.runtime.config.current() as unknown as OpenClawConfig, to: target, text: claim.body, accountId, deliveryQueueId: eventKey });
        ...
        await withState((state) => {
          ...
          if (messageId) current.completionWakeAt = nowIso();
          current.updatedAt = nowIso();
        });
      } catch (error) {
        await withState((state) => {
          ...
          const current = state.plans.find((candidate) => candidate.id === planId);
          if (!item || !current) return;
          item.status = "unknown"; item.error = safeError(error); item.updatedAt = nowIso();
          current.completionNotificationStatus = "unknown"; current.completionNotificationError = item.error; current.updatedAt = nowIso();
        });
      }
    };

    const refreshParents = async (planId: string) => {
      const terminalNow = await withState((state) => {
        const plan = state.plans.find((item) => item.id === planId);
        if (!plan || terminal(plan.status)) return Boolean(plan && terminal(plan.status));
        for (const packet of state.packets.filter((item) => item.planId === planId && item.workUnitIds.length)) {
          if (terminal(packet.status)) continue;
          const units = packet.workUnitIds.map((id) => state.workUnits.find((item) => item.id === id)).filter((item): item is WorkUnitRecord => Boolean(item));
          if (units.some((unit) => ["failed", "blocked", "cancelled"].includes(unit.status))) packet.status = "blocked";
          else if (units.length && units.every((unit) => unit.status === "succeeded")) packet.status = "succeeded";
          else if (units.some((unit) => unit.status === "reconcile_required")) packet.status = "reconcile_required";
          else if (units.some((unit) => unit.status === "running")) packet.status = "running";
          else packet.status = "pending";
          packet.updatedAt = nowIso();
        }
        const packets = plan.packetIds.map((id) => state.packets.find((item) => item.id === id)).filter((item): item is PacketRecord => Boolean(item));
        if (packets.some((packet) => packet.status === "blocked")) plan.status = "blocked";
        else if (packets.length && packets.every((packet) => packet.status === "succeeded")) plan.status = "succeeded";
        else if (packets.some((packet) => packet.status === "running")) plan.status = "running";
        else plan.status = "planned";
        const inbox = state.inbox.find((item) => item.id === plan.inboxId);
        if (inbox) inbox.status = plan.status === "succeeded" ? "completed" : plan.status === "blocked" ? "blocked" : packets.some((packet) => packet.status === "reconcile_required") ? "reconcile_required" : plan.status === "running" ? "running" : "delegating";
        plan.updatedAt = nowIso();
        return terminal(plan.status);
      });
      if (terminalNow) await sendPlanCompletion(planId);
    };

    const applyObservedEnd = (state: TaskSystemState, link: LinkRecord, outcome: string, error?: string) => {
      ...
      if (outcome === "ok" && link.proofReceipt && link.completionReceipt) {
        ...
        if (unit && !terminal(unit.status)) { unit.status = "succeeded"; unit.resultRef = link.completionReceipt; unit.updatedAt = nowIso(); }
      } else if (outcome === "ok") {
        link.status = "reconcile_required"; link.error = "执行已结束，正在核对最后收据";
        if (unit && !terminal(unit.status)) { unit.status = "reconcile_required"; unit.error = link.error; unit.updatedAt = nowIso(); }
      } else {
        link.status = "failed"; link.error = safeError(error ?? `worker ended as ${outcome || "unknown"}`);
        if (unit && !terminal(unit.status)) { unit.status = "failed"; unit.error = link.error; unit.updatedAt = nowIso(); }
      }
    };

    const materializePacket = async (packetId: string) => withState((state) => {
      const packet = state.packets.find((item) => item.id === packetId);
      if (!packet || terminal(packet.status) || packet.workUnitIds.length) return packet?.workUnitIds[0];
      ...
      packet.workUnitIds.push(unitId); packet.delegationMode = "direct_subagent_v3"; packet.status = "pending"; packet.decompositionReceipt = sha256(JSON.stringify({ packetId: packet.id, unitId, mode: "direct_subagent_v3" })); packet.updatedAt = nowIso();
      return unitId;
    });

    const startDirectUnit = async (unitId: string) => {
      const unitSnapshot = (await readState()).workUnits.find((item) => item.id === unitId);
      if (!unitSnapshot || unitSnapshot.status !== "ready") return false;
      if (!receiptToolsAllowed(unitSnapshot.agentId)) {
        await withState((state) => {
          const unit = state.workUnits.find((item) => item.id === unitId);
          if (unit && !terminal(unit.status)) { unit.status = "failed"; unit.error = "目标角色缺少任务收据工具，未启动执行"; unit.updatedAt = nowIso(); }
        });
      ...
      const claim = await withState((state) => {
        const unit = state.workUnits.find((item) => item.id === unitId);
        if (!unit || unit.status !== "ready" || state.links.some((item) => item.workUnitId === unitId)) return undefined;
      ...
      try {
        ...
        if (!started.runId) throw new Error("subagent returned no runId");
        const accepted = await withState((state) => {
          ...
          if (!link || terminal(link.status)) return { terminalMerged: false, planId: claim.unit.planId };
          if ((link.runId && link.runId !== started.runId) || (link.observedEndedRunId && link.observedEndedRunId !== started.runId)) {
            link.status = "reconcile_required"; link.error = "启动回执与已观察事件不一致"; link.updatedAt = nowIso();
            const unit = state.workUnits.find((item) => item.id === link.workUnitId);
            if (unit && !terminal(unit.status)) { unit.status = "reconcile_required"; unit.error = link.error; unit.updatedAt = nowIso(); }
            return { terminalMerged: false, planId: link.planId };
          }
          link.runId = started.runId; link.status = "running"; link.updatedAt = link.lastActivityAt = nowIso();
          const unit = state.workUnits.find((item) => item.id === link.workUnitId);
          if (unit && !terminal(unit.status)) { unit.status = "running"; unit.updatedAt = nowIso(); }
          if (link.observedEndedRunId) applyObservedEnd(state, link, link.observedEndedOutcome ?? "ended", link.observedEndedError);
        ...
        if (accepted.terminalMerged) {
          ...
          if (!terminal(latest.plans.find((item) => item.id === accepted.planId)?.status ?? "blocked")) await startReadyPackets(accepted.planId, true);
        }
        return true;
      } catch (error) {
        await withState((state) => {
          ...
          if (link && !terminal(link.status)) { link.status = "reconcile_required"; link.error = safeError(error); link.updatedAt = nowIso(); }
          if (unit && !terminal(unit.status)) { unit.status = "reconcile_required"; unit.error = "启动结果未知，禁止自动重试"; unit.updatedAt = nowIso(); }
        });
    ...
    const startReadyPackets = async (planId: string, launch = true) => {
      const snapshot = await readState();
      for (const packet of snapshot.packets.filter((item) => item.planId === planId && ["waiting_dependencies", "pending"].includes(item.status))) {
        if (packet.delegationMode === "legacy_workboard_v2" || (packet.delegationMode === "legacy_controller_v2" && !packet.decompositionReceipt)) continue;
        const dependencies = packet.dependsOn.map((key) => snapshot.packets.find((item) => item.planId === planId && item.packetKey === key));
        if (dependencies.some((item) => !item || item.status !== "succeeded")) continue;
        await withState((state) => {
          const current = state.packets.find((item) => item.id === packet.id);
          if (!current || terminal(current.status)) return;
          current.inputRefs = dependencies.flatMap((dep) => dep?.workUnitIds.map((id) => state.workUnits.find((unit) => unit.id === id)?.resultRef).filter((value): value is string => Boolean(value)) ?? []);
          current.delegationMode = current.delegationMode ?? "direct_subagent_v3"; current.status = "pending"; current.updatedAt = nowIso();
        });
        const unitId = await materializePacket(packet.id);
        if (launch && unitId) await startDirectUnit(unitId);
      }
      await refreshParents(planId);
    };

    const reconcilePlan = async (planId: string) => {
      await withState((state) => {
        for (const link of state.links.filter((item) => item.planId === planId && item.dispatchKind === "subagent_direct" && item.status === "reconcile_required" && item.workerOutcome === "ok")) {
          if (link.proofReceipt && link.completionReceipt) continue;
          link.status = "blocked"; link.error = "执行已结束但显式核对后仍缺少完整收据"; link.updatedAt = nowIso();
          const unit = state.workUnits.find((item) => item.id === link.workUnitId);
          if (unit && !terminal(unit.status)) { unit.status = "blocked"; unit.error = link.error; unit.updatedAt = nowIso(); }
        }
        for (const packet of state.packets.filter((item) => item.planId === planId && !terminal(item.status))) {
          ...
          if (links.some((item) => item.cardId || item.dispatchKind === "workboard_legacy")) { packet.delegationMode = "legacy_workboard_v2"; continue; }
          const leaseLive = Boolean(packet.controllerLeaseUntil && Date.parse(packet.controllerLeaseUntil) > Date.now());
          if (packet.controllerRunId && leaseLive) { packet.delegationMode = "legacy_controller_v2"; continue; }
          if (packet.controllerRunId && !packet.decompositionReceipt) { packet.status = "reconcile_required"; packet.error = "旧控制会话结果未知，禁止自动重放"; continue; }
          ...
          for (const id of packet.workUnitIds) {
            ...
            if (!unit || state.links.some((item) => item.workUnitId === id) || terminal(unit.status)) continue;
            const rule = capabilities.get(packet.capability);
            if (!rule || !rule.owners.includes(packet.targetAgentId)) {
              unit.status = "reconcile_required"; unit.error = "旧任务包的角色与能力不匹配，已隔离且不会启动"; unit.updatedAt = nowIso();
              packet.status = "reconcile_required"; packet.error = unit.error; packet.updatedAt = nowIso();
              continue;
            }
            if (isLegacyPlaceholder(unit)) {
              ...
              unit.status = "reconcile_required"; unit.error = "旧工作单元不是已知占位模板，且角色或能力与任务包不一致，已隔离"; unit.updatedAt = nowIso();
              packet.status = "reconcile_required"; packet.error = unit.error; packet.updatedAt = nowIso();
              continue;
            }
            unit.delegationMode = "direct_subagent_v3"; unit.status = "ready"; unit.updatedAt = nowIso();
          }
        }
      });
      await startReadyPackets(planId, true);
      const state = await readState();
      for (const unit of state.workUnits.filter((item) => item.planId === planId && item.status === "ready")) await startDirectUnit(unit.id);
    ...
    api.on("before_agent_run", async (event, ctx) => {
      const context = ctx as unknown as JsonRecord;
      if (!ctx.agentId || !accountByAgent.has(ctx.agentId) || !ctx.sessionKey) return;
      if (providerOf(context) !== "telegram" || event.senderIsOwner !== true) return;
      if (ownerTelegramId && String(event.senderId ?? ctx.senderId ?? "") !== ownerTelegramId) return;
      ...
      const eventKey = `${route}:${String((event as unknown as JsonRecord).messageId ?? ctx.runId ?? `${Math.floor(Date.now() / 30_000)}:${sha256(prompt).slice(0, 20)}`)}`;
      const inbox = await withState((state) => {
        ...
        state.inbox.push(created); return structuredClone(created);
      });
      const key = turnKey(context, event as unknown as JsonRecord);
      if (key) turnInbox.set(key, inbox.id);
    });

    api.on("agent_turn_prepare", async (_event, ctx) => {
      if (!ctx.sessionKey) return;
      const state = await readState();
      const inboxId = turnInbox.get(turnKey(ctx as unknown as JsonRecord));
    ...
    api.on("before_prompt_build", async (_event, ctx) => {
      if (!ctx.sessionKey || sessionKind(ctx.sessionKey, controllers) !== "controller") return;
    ...
    api.on("before_tool_call", async (event, ctx) => {
      ...
      if (kind === "worker" || kind === "automation") return;
      const key = turnKey(ctx as unknown as JsonRecord);
    ...
    api.on("before_agent_finalize", async (event, ctx) => {
      ...
      if (!delegated && !event.stopHookActive) {
        return { action: "revise" as const, reason: "delegation_receipt_missing", retry: { instruction: "本轮尚未形成真实分工。立即调用 task_delegate plan，至少创建一个角色任务包；不得亲自执行或要求少主开放权限。", idempotencyKey: `delegate:${inbox.id}`, maxAttempts: 1 } };
      }
      ...
      while (associations.length > 1000) associations.shift();
    });
    ...
    api.on("message_sending", async (event, ctx) => {
      ...
      if (matches.length !== 1) return { cancel: true, cancelReason: "ambiguous task turn correlation" };
      const match = matches[0];
      if (match.delegated) return;
      ...
      if (success && event.toolName === "morning_brief_control") {
        ...
        await withState((state) => {
          ...
          for (const module of modules) {
            ...
            if (expected) { expected.status = "applied"; expected.receiptHash = sha256(JSON.stringify(detailsOf(event.result))); expected.updatedAt = nowIso(); }
          }
        });
      }
    });
    ...
    api.on("subagent_spawned", async (event) => {
      const matched = await withState((state) => {
        ...
        if (!link || terminal(link.status) || !link.generation || !link.launchNonce) return undefined;
        if (link.runId && link.runId !== event.runId) { link.status = "reconcile_required"; link.error = "启动事件与回执不一致"; return { planId: link.planId, ok: false }; }
        link.runId = event.runId; link.status = "running"; link.updatedAt = link.lastActivityAt = nowIso();
        const unit = state.workUnits.find((item) => item.id === link.workUnitId);
        if (unit && !terminal(unit.status)) { unit.status = "running"; unit.updatedAt = nowIso(); }
        return { planId: link.planId, ok: true };
      });
      if (matched) await refreshParents(matched.planId);
    });
    ...
    api.on("subagent_ended", async (event) => {
      ...
      const controllerPacket = await withState((state) => {
        ...
        if (!packet.decompositionReceipt) { packet.status = "blocked"; packet.error = "controller ended without a decomposition receipt"; packet.updatedAt = nowIso(); }
        return { id: packet.id, planId: packet.planId };
      });
      if (controllerPacket) { await refreshParents(controllerPacket.planId); return; }
      ...
      const ended = await withState((state) => {
        ...
        if (!current || terminal(current.status) || current.generation !== link.generation || current.launchNonce !== link.launchNonce) return { finalize: false };
        if (!runId) {
          current.status = "reconcile_required"; current.error = "结束事件缺少 runId，已隔离且不会重跑"; current.updatedAt = nowIso();
          const unit = state.workUnits.find((item) => item.id === current.workUnitId);
          if (unit && !terminal(unit.status)) { unit.status = "reconcile_required"; unit.error = current.error; unit.updatedAt = nowIso(); }
          return { finalize: true };
        }
        if (current.runId && current.runId !== runId) {
          current.status = "reconcile_required"; current.error = "结束事件与启动回执不一致"; current.updatedAt = nowIso();
          const unit = state.workUnits.find((item) => item.id === current.workUnitId);
          if (unit && !terminal(unit.status)) { unit.status = "reconcile_required"; unit.error = current.error; unit.updatedAt = nowIso(); }
          return { finalize: true };
        }
        const outcome = typeof eventRecord.outcome === "string" ? eventRecord.outcome : "ended";
        current.observedEndedRunId = runId; current.observedEndedOutcome = outcome;
    ...
    api.registerTool((ctx) => {
      if (!ctx.agentId || !accountByAgent.has(ctx.agentId) || !ctx.sessionKey) return null;
        ...
        async execute(_id: string, raw: unknown) {
          ...
          try {
            if (p.action === "inspect") {
              ...
              if (!plan) throw new Error("没有找到对应任务计划");
            ...
            if (p.action === "block" || p.action === "cancel") {
              ...
              await withState((state) => {
                ...
                plan.status = nextStatus; plan.error = reason; plan.updatedAt = nowIso();
                const inbox = state.inbox.find((item) => item.id === plan.inboxId);
                if (inbox && !terminal(inbox.status)) { inbox.status = nextStatus; inbox.error = reason; inbox.updatedAt = nowIso(); }
                for (const packet of state.packets.filter((item) => item.planId === planId && !terminal(item.status))) { packet.status = nextStatus; packet.error = reason; packet.updatedAt = nowIso(); }
                for (const unit of state.workUnits.filter((item) => item.planId === planId && !terminal(item.status))) { unit.status = nextStatus; unit.error = reason; unit.updatedAt = nowIso(); }
                for (const link of state.links.filter((item) => item.planId === planId && !terminal(item.status))) { link.status = nextStatus; link.error = reason; link.updatedAt = nowIso(); }
              });
            ...
            if (p.action === "plan") {
              ...
              if (kind !== "main" && !(kind === "controller" && ctx.agentId === "housekeeper" && handoffId)) throw new Error("only a role main session or the housekeeper handoff controller can create a top-level plan");
              const state = await readState();
              const handoff = handoffId ? state.handoffs.find((item) => item.id === handoffId) : undefined;
              if (handoffId && (!handoff || handoff.targetAgentId !== "housekeeper" || !["pending", "applying"].includes(String(handoff.status)))) throw new Error("没有可应用的跨域转交");
              ...
              if (!inbox) throw new Error("跨域转交缺少原始任务入口，不能创建孤立计划");
              if (terminal(inbox.status)) throw new Error("该任务入口已经结束，不能重新用于新计划");
              if (inbox.profile === "conversation") throw new Error("普通聊天不应创建执行计划");
              const titleValue = text(p.title, "title", 240); const goal = text(p.goal, "goal");
              if (!Array.isArray(p.packets) || !p.packets.length) throw new Error("packets is required");
              const packetInputs = p.packets.map(record);
              const keys = packetInputs.map((item) => text(item.packet_key, "packet_key", 100));
              if (new Set(keys).size !== keys.length) throw new Error("packet_key must be unique");
              ...
              const created = await withState((next) => {
                ...
                if (existingPlan) return structuredClone(existingPlan);
                const timestamp = nowIso(); const planId = `plan-${randomUUID()}`;
                const packetIds: string[] = [];
                for (let index = 0; index < packetInputs.length; index += 1) {
                  const item = packetInputs[index]; const target = text(item.target_agent, "target_agent", 80);
                  if (!accountByAgent.has(target)) throw new Error(`unknown target agent ${target}`);
                  if (ctx.agentId !== "housekeeper" && target !== ctx.agentId && target !== "housekeeper") throw new Error("跨角色任务必须先转给賈南風，不得直接指派其他专业角色");
                  const capability = text(item.capability, "capability", 160); capabilityRule(capability, target);
                  const packetId = `packet-${randomUUID()}`; packetIds.push(packetId);
                ...
                const plan: PlanRecord = { id: planId, inboxId: inbox.id, sourceAgentId: ctx.agentId!, sourceSessionKey: ctx.sessionKey!, title: titleValue, goal, profile: inbox.profile, delegationMode: "direct_subagent_v3", status: "planned", packetIds, createdAt: timestamp, updatedAt: timestamp };
                next.plans.push(plan);
                const currentInboxRecord = next.inbox.find((item) => item.id === inbox.id)!;
                currentInboxRecord.planIds.push(planId); currentInboxRecord.goal = goal; currentInboxRecord.status = "delegating"; currentInboxRecord.updatedAt = timestamp;
                if (handoffId) {
                  const currentHandoff = next.handoffs.find((item) => item.id === handoffId);
                  if (currentHandoff) { currentHandoff.status = "applied"; currentHandoff.planId = planId; currentHandoff.updatedAt = timestamp; }
                }
            ...
            if (p.action === "decompose_packet") {
              if (sessionKind(toolSessionKey, controllers) !== "controller") throw new Error("only a role controller session can decompose a packet");
              const packetId = text(p.packet_id, "packet_id", 100);
              if (!Array.isArray(p.work_units) || !p.work_units.length) throw new Error("work_units is required");
              const unitInputs = p.work_units.map(record);
              const keys = unitInputs.map((item) => text(item.unit_key, "unit_key", 100));
              if (new Set(keys).size !== keys.length) throw new Error("unit_key must be unique");
              ...
              const planId = await withState((state) => {
                const packet = state.packets.find((item) => item.id === packetId);
                if (!packet) throw new Error("没有找到对应角色任务包");
                if (packet.delegationMode !== "legacy_controller_v2") throw new Error("direct task packets cannot be decomposed again");
                if (packet.targetAgentId !== ctx.agentId || packet.controllerSessionKey !== ctx.sessionKey) throw new Error("当前角色不能分解这个任务包");
                if (packet.decompositionReceipt) return packet.planId;
                const timestamp = nowIso();
                for (let index = 0; index < unitInputs.length; index += 1) {
                  const item = unitInputs[index]; const capability = text(item.capability, "unit.capability", 160);
                  capabilityRule(capability, ctx.agentId!);
                  const unitId = `unit-${randomUUID()}`; const deps = dependencyMap.get(keys[index]) ?? [];
                ...
                packet.decompositionReceipt = sha256(JSON.stringify({ packetId: packet.id, units: keys, agentId: ctx.agentId })); packet.status = "running"; packet.updatedAt = timestamp;
                const plan = state.plans.find((item) => item.id === packet.planId); if (plan) { plan.status = "running"; plan.updatedAt = timestamp; }
                const inbox = plan ? state.inbox.find((item) => item.id === plan.inboxId) : undefined; if (inbox) inbox.status = "running";
                return packet.planId;
              });
              for (const unit of (await readState()).workUnits.filter((item) => item.planId === planId && item.status === "ready")) await startDirectUnit(unit.id);
          ...
          } catch (error) { return toolResult({ ok: false, error: error instanceof Error ? error.message : String(error) }); }
        }
      };
    ...
    api.registerTool((ctx) => {
      ...
      return {
        ...
        async execute(_id: string, raw: unknown) {
          const p = record(raw);
          try {
            if (p.action === "list") { const state = await readState(); return toolResult({ ok: true, items: state.inbox.filter((item) => item.sessionKey === ctx.sessionKey).slice(-20).map((item) => ({ status: item.status, profile: item.profile, hasPlan: item.planIds.length > 0 })) }); }
            if (p.action === "inspect") { const state = await readState(); const inbox = currentInbox(state, toolSessionKey, p.inbox_id); return toolResult({ ok: true, item: { status: inbox.status, profile: inbox.profile, hasPlan: inbox.planIds.length > 0 } }); }
            if (p.action === "reconcile") { const state = await readState(); const inbox = currentInbox(state, toolSessionKey, p.inbox_id); const summaries = []; for (const planId of inbox.planIds) summaries.push(await reconcilePlan(planId)); const unresolved = summaries.reduce((sum, item) => sum + item.unresolved, 0); return toolResult({ ok: unresolved === 0, reconciled: unresolved === 0, unresolved, summaries }); }
            if (p.action === "triage") {
              const profile = p.profile as WorkflowProfile; if (!PROFILE_VALUES.includes(profile)) throw new Error("profile is required");
              await withState((state) => { const inbox = currentInbox(state, toolSessionKey, p.inbox_id); if (terminal(inbox.status)) throw new Error("该任务入口已经结束，不能重新分流"); inbox.profile = profile; inbox.goal = typeof p.goal === "string" ? p.goal : inbox.goal; inbox.status = "triaged"; inbox.updatedAt = nowIso(); });
              return toolResult({ ok: true, triaged: true, instruction: profile === "conversation" ? "直接自然回复" : "立即调用 task_delegate plan" });
            }
            if (p.action === "start") return toolResult({ ok: false, error: "旧的单卡 start 已停用；请用 task_delegate plan 明确分工" });
            if (p.action === "block") { const reason = text(p.reason, "reason", 2000); await withState((state) => { const inbox = currentInbox(state, toolSessionKey, p.inbox_id); inbox.status = "blocked"; inbox.error = reason; inbox.updatedAt = nowIso(); }); return toolResult({ ok: true, blocked: true }); }
            if (p.action === "queue_notification") {
              const eventKey = text(p.event_key, "event_key", 240); const contentHash = text(p.content_hash, "content_hash", 64).toLowerCase();
              if (eventKey.startsWith("task-plan-completion:")) throw new Error("完成通知命名空间只允许插件直发通道使用");
              await withState((state) => { if (!state.outbox.some((item) => item.eventKey === eventKey)) state.outbox.push({ id: `outbox-${randomUUID()}`, eventKey, kind: "ordinary_agent", sessionKey: ctx.sessionKey!, contentHash, status: "pending", createdAt: nowIso(), updatedAt: nowIso() }); });
    ...
    api.registerTool((ctx) => {
      ...
      return {
        ...
        async execute(_id: string, raw: unknown) {
          ...
          try {
            if (p.action === "submit") {
              const capability = text(p.capability, "capability", 160); const timestamp = nowIso(); const id = `handoff-${randomUUID()}`;
              await withState((state) => {
                const inbox = currentInbox(state, toolSessionKey);
                if (terminal(inbox.status)) throw new Error("该任务入口已经结束，不能重新转交");
              ...
              await withState((state) => { const item = state.handoffs.find((candidate) => candidate.id === id); if (item) { item.status = "applying"; item.runId = started.runId; item.updatedAt = nowIso(); } });
              return toolResult({ ok: true, routed: true, status: "applying" });
            }
            const id = text(p.handoff_id, "handoff_id", 100); const state = await readState(); const item = state.handoffs.find((candidate) => candidate.id === id); if (!item) throw new Error("没有找到转交记录");
            if (p.action === "status") return toolResult({ ok: true, status: item.status });
            if (p.action === "cancel") { await withState((next) => { const current = next.handoffs.find((candidate) => candidate.id === id); if (current) { current.status = "cancelled"; current.updatedAt = nowIso(); } }); return toolResult({ ok: true, cancelled: true }); }
    ...
    api.registerTool((ctx) => {
      ...
      return {
        ...
        async execute(_id: string, raw: unknown) {
          const p = record(raw);
          try {
            if (p.action === "catalog") return toolResult({ ok: true, modules: MODULE_VALUES.map((module) => ({ module, label: MODULE_LABELS[module] })) });
            const state = await readState(); const inbox = currentInbox(state, toolSessionKey, p.inbox_id);
            if (p.action === "inspect") return toolResult({ ok: true, modules: inbox.expectedModules.map((item) => ({ label: MODULE_LABELS[item.module], status: item.status })) });
            if (p.action === "expect") {
              const module = text(p.module, "module", 100); const action = text(p.control_action, "control_action", 100);
              await withState((next) => { const current = currentInbox(next, ctx.sessionKey!, inbox.id); if (terminal(current.status)) throw new Error("该任务入口已经结束，不能追加模块"); if (!current.expectedModules.some((item) => item.module === module && item.action === action)) current.expectedModules.push({ module, action, status: "expected", toolName: "morning_brief_control", updatedAt: nowIso() }); });
    ...
    const receiptFactory = (kind: "proof" | "complete") => (ctx: { agentId?: string; sessionKey?: string }) => {
      if (!ctx.agentId || !ctx.sessionKey || sessionKind(ctx.sessionKey, controllers) !== "worker") return null;
      return {
        ...
        async execute(_id: string, raw: unknown) {
          ...
          try {
            ...
            const changed = await withState((state) => {
              ...
              const unit = state.workUnits.find((item) => item.id === unitId);
              if (!link || !unit) throw new Error("收据与当前执行不匹配");
              if (terminal(link.status) || terminal(unit.status)) throw new Error("任务已经进入终态，拒绝迟到收据");
              const receipt = sha256(JSON.stringify({ unitId, generation, nonce, kind, summary }));
              if (kind === "proof") { link.proofReceipt = receipt; link.proofSummary = summary; }
              else { link.completionReceipt = receipt; link.completionSummary = summary; }
              link.updatedAt = link.lastActivityAt = nowIso();
              if (link.workerOutcome === "ok" && link.proofReceipt && link.completionReceipt) {
                link.status = "succeeded"; unit.status = "succeeded"; unit.resultRef = link.completionSummary; unit.updatedAt = nowIso();
              }
              return { planId: link.planId, succeeded: link.status === "succeeded" };
            });
            await refreshParents(changed.planId);
            if (changed.succeeded) {
              try { await startReadyPackets(changed.planId, true); } catch { /* state records the launch result */ }
            }
            return toolResult({ ok: true, recorded: kind });
          } catch (error) { return toolResult({ ok: false, error: safeError(error) }); }
        }
      };
    };
    ...
    api.on("message_sent", async (event, ctx) => {
      ...
      await withState((state) => {
        ...
        if (!item) return;
        item.status = event.success ? event.messageId ? "sent" : "unknown" : "failed"; item.messageId = event.messageId; item.error = event.error; item.updatedAt = nowIso();
      });
    });
  }
});
