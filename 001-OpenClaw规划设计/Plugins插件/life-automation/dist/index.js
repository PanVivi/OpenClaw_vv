...
const ParamsSchema = Type.Object({
    ...
    schedule_kind: Type.Optional(Type.Union([
        Type.Literal("cron"),
        Type.Literal("every"),
        Type.Literal("at")
    ])),
...
function scheduleFromEveryMinutes(minutes, timezone) {
    ...
    if (minutes === 10080)
        return { kind: "cron", expr: "0 0 * * 0", tz: timezone };
    throw new Error("every_minutes must map exactly to Cron; otherwise use schedule_kind=cron");
}
function buildSchedule(params, defaultTimezone) {
    ...
    if (kind === "at") {
        const schedule = { kind: "at", at: requireText(params.at, "at") };
        computeNextRunAt(schedule, Date.now());
        return schedule;
    }
    throw new Error("schedule_kind must be cron, every, or at");
}
function computeNextRunAt(schedule, fromMs) {
    if (schedule.kind === "at") {
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        const allowedAgentId = typeof pluginConfig.agentId === "string" && pluginConfig.agentId.trim()
            ...
            : "life";
        const defaultTimezone = typeof pluginConfig.timezone === "string" && pluginConfig.timezone.trim()
        ...
        const ownerChatId = typeof pluginConfig.ownerChatId === "string" && pluginConfig.ownerChatId.trim()
            ...
            : undefined;
        const telegramAccountId = typeof pluginConfig.telegramAccountId === "string" &&
        ...
        const sessionKey = ownerChatId
            ...
            : undefined;
        const statePath = join(process.env.HOME ?? api.rootDir ?? ".", ".openclaw", "life-automation", "jobs.json");
        ...
        const executeRecord = async (record, runKey) => {
            if (!sessionKey)
            ...
            try {
                const due = await withStateLock(async () => {
                    ...
                    for (const job of state.jobs) {
                        ...
                        if (!job.enabled || !job.nextRunAt || Date.parse(job.nextRunAt) > now)
                            continue;
                ...
                for (const snapshot of due) {
                    ...
                    await withStateLock(async () => {
                        ...
                        if (!current || current.pendingRunAt !== snapshot.pendingRunAt)
                            return;
                ...
                api.logger.error(`life-automation scheduler tick failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            finally {
                ticking = false;
            }
        ...
        api.on("gateway_start", () => {
            ...
            tickTimer.unref?.();
            void processDue();
            api.logger.info(`life-automation scheduler started (state=${statePath}, account=${telegramAccountId})`);
        });
        api.on("gateway_stop", () => {
            if (tickTimer)
                clearInterval(tickTimer);
        ...
        api.registerTool((ctx) => {
            ...
            return {
                ...
                description: "Manage only life-owned persistent scheduled tasks. Supports inspect/list/get/create/update/pause/resume/remove/run_now. It cannot run shell commands, write OpenClaw config, or target another agent.",
                ...
                async execute(_toolCallId, rawParams) {
                    ...
                    try {
                        return await withStateLock(async () => {
                            const state = await readState();
                            const findRecord = () => {
                                const identifier = typeof params.task_id === "string" && params.task_id.trim()
                                    ? params.task_id.trim()
                                    : requireText(params.job_id, "job_id or task_id");
                                const record = state.jobs.find((candidate) => candidate.taskId === identifier ||
                            ...
                            switch (params.action) {
                                case "inspect": {
                                    ...
                                    const inspectedJob = identifier
                                        ? state.jobs.find((candidate) => candidate.taskId === identifier ||
                                            candidate.schedulerJobId === identifier)
                                        : undefined;
                                ...
                                case "create": {
                                    ...
                                    if (existing)
                                        return result({ ok: true, created: false, job: existing });
                                ...
                                case "update": {
                                    ...
                                    const scheduleUpdateRequested = typeof params.schedule_kind === "string" ||
