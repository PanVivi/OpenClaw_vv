...
function pathSegments(value: unknown, allowEmpty: boolean) {
...
function confinedTarget(ownerRoot: string, segments: string[]) {
...
function fileTarget(ownerRoot: string, value: unknown) {
...
async function directoryOnly(target: string) {
...
async function regularFileOnly(target: string) {
  const info = await lstat(target);
...
async function ensureDirectories(ownerRoot: string, segments: string[]) {
...
async function verifyExistingPath(ownerRoot: string, segments: string[]) {
  await directoryOnly(ownerRoot);
  let current = ownerRoot;
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    const configuredOwnerRoot =
      typeof pluginConfig.ownerRoot === "string" && pluginConfig.ownerRoot.trim()
    ...
    if (
      typeof pluginConfig.ownerRoot === "string" &&
      pluginConfig.ownerRoot.trim() &&
    ...
    api.registerTool(
      (ctx) => {
        ...
        return {
          ...
          description:
            "Manage only UTF-8 life files under the owner's dedicated folder. Use list/get/mkdir/create/update/append. Paths must be relative and use supported text extensions. This tool cannot delete, execute, or access anything outside that folder.",
