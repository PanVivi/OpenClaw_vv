...
type CaptureStatus = "unclaimed" | "claimed";
type CaptureRecord = {
  id: string;
  fingerprint: string;
  tokenFile: string;
...
type InboxState = { version: 1; captures: CaptureRecord[] };
...
const ParamsSchema = Type.Object(
  {
    ...
    agent_id: Type.Optional(
      Type.Union(TARGETS.map((target) => Type.Literal(target)))
    )
  },
...
function senderMatches(value: string, ownerUserId: string): boolean {
...
export default definePluginEntry({
  ...
  description:
    "Capture owner-supplied Telegram Bot Tokens before redaction and give ops opaque token-file handles.",
  register(api) {
    ...
    const allowedAgentId =
      typeof pluginConfig.agentId === "string" && pluginConfig.agentId.trim()
        ...
        : "ops";
    const allowedAccountId =
      typeof pluginConfig.accountId === "string" && pluginConfig.accountId.trim()
    ...
    const readState = async (): Promise<InboxState> => {
      try {
        const value = JSON.parse(await readFile(statePath, "utf8")) as InboxState;
    ...
    const writeState = async (state: InboxState) => {
    ...
    const withStateLock = async <T>(operation: () => Promise<T>): Promise<T> => {
      ...
      try {
        ...
        if (!acquired) throw new Error("Timed out acquiring ops token inbox lock");
        try {
    ...
    api.on("message_received", async (event, ctx) => {
      ...
      for (const token of new Set(tokens)) {
        try {
          ...
          api.logger.error(
            `ops token intake capture failed without exposing the credential: ${
    ...
    api.registerTool(
      (ctx) => {
        ...
        return {
          ...
          description:
            "List, claim, or discard owner-supplied Telegram Bot Tokens captured before redaction. Never asks the owner to resend a token that appears in this inbox and never reads or returns token contents. claim returns a 0600 tokenFile path for native OpenClaw channels add/configuration.",
            ...
            try {
              return await withStateLock(async () => {
                ...
                if (
                  ...
                  !TARGETS.includes(params.agent_id as (typeof TARGETS)[number])
                ) {
                  throw new Error(`agent_id must be one of: ${TARGETS.join(", ")}`);
                }
                if (capture.status === "claimed" && capture.agentId !== params.agent_id) {
                  throw new Error(
                    `Capture is already claimed by ${capture.agentId ?? "another agent"}`
                  );
                }
                ...
                return toolResult({
                  ...
                  mode: "0600",
                  instruction:
                    "Use this file with native OpenClaw tokenFile/channels add. Do not read, echo, copy into command arguments, logs, transcripts, or memory."
                });
              });
            } catch (error) {
              return toolResult({
