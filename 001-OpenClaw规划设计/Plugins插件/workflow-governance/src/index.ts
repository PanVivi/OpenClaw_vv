        ...
        async execute(_id: string, raw: unknown) {
          ...
          try {
            ...
            if (p.action === "begin_execution") {
              if (!state.planHash || state.reviews.length !== 3 || state.reviews.some((r, i) => r.planHash !== state.planHash || r.reviewNumber !== i + 1)) throw new Error("execution requires three independent complete reviews of the current plan");
