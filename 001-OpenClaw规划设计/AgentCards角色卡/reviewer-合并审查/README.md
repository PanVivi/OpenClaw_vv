# reviewer｜夏姬（合并审查）｜当前角色卡 v0.04｜CANDIDATE 候选版
...
1. 五个 workspace 文件均来自 v0.04 固定提交，SHA-256 与部署清单一致。
...
专用 Stage/Gate 持久化、目标 Generation、硬单次消费、跨重启恢复和精确 A2A 分别验收。未完成项不得虚构，但不阻塞基础 reviewer 上线。
