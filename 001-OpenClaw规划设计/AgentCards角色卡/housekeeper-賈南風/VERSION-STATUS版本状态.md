...
| v1.16 | `CANDIDATE` | NAS 回滚基线 | 完整继承 v1.15，新增晨报相关信息转交入口 |
| v1.17 | `CANDIDATE` | NAS 当前部署 | 完整继承 v1.16，扩展为全部晨报模块的持久转交，并以 life 的 applied 回执作为唯一完成判定 |

v1.02 仍是最近 `STABLE` 角色基线；v1.17 是当前设计版本，实际部署以 `DeploymentStatus部署状态.md` 为准。
