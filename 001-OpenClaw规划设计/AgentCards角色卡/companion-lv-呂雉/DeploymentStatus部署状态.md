...
- 身份仍为 `companion-lv`，Telegram account/binding 仍为 `companion-lv`，人格和非工程边界未变。
...
- 当前设计与实际部署版本：v0.08 `CANDIDATE`；冷酷命令型陪伴人格和非工程边界未变。
- 共同协议 v0.08 已部署：内部协调字段不进入陪伴回复；默认以“本宫/少主”冷硬、直接地回应。
