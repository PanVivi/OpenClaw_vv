# 001 OpenClaw架设部署｜FinalDesign 最终设计｜v1.10
...
## 15. 2026-07-23 生产验收增量

- housekeeper 正式委派包必须携带原始授权来源、Task ID、目标、范围、完成标准、风险等级、责任 Agent、首次回报期限和回报目标；字段完整且范围未变化时，责任 Agent不得再次向少主索要同一任务授权。
- `housekeeper-async-dispatch 1.2.2` 使跨角色派发立即返回，并持久记录首次回报期限。下游 progress / completed / blocked / cancelled 自动回写；到期未回报时由同角色 housekeeper 子 Agent主动向原会话报告，且只能陈述“未按期回报、执行状态未知”，不得把未知状态说成卡死或失败。
- 凭据不从 transcript、日志、工具回显或 `config get` 恢复。`ops-token-intake 1.0.1` 仅在少主发给 ops 的 Telegram 原始入站消息中捕获 Bot Token，保存为 `0600 tokenFile`，只把 opaque handle 交给 ops；实际 account/binding 仍使用 OpenClaw 原生能力。
- 任务跟踪和凭据索引均采用跨实例文件锁、唯一临时文件和原子替换；监控状态写入异常必须 fail-open，不得阻断实际 A2A 委派。
- 本增量不授予 housekeeper shell 或凭据权限，不开放跨 Agent历史，不改变“跨角色 A2A、同角色 spawn”的边界。

## 16. v1.09：正式任务控制面

自 2026-07-24 起，正式业务任务的权威状态改为 OpenClaw 官方 Workboard；原 `housekeeper-async-dispatch` 已停用但历史文件保留。

- 賈南風仍是唯一用户侧总调度：接单、风险判断、建卡、拆分、依赖、指派和汇总，不亲自长期执行。
- 正式卡片必须保存目标、范围、原始授权、风险、责任 Agent、完成标准、幂等键和必要依赖；已授权范围内的低中风险后续步骤不重复索权。
- 角色 worker 由 Workboard 派发到独立 Agent session；主 Telegram 会话不等待 worker，任务事实以 card、claim、heartbeat、attempt、proof、Task/Task Flow 为准。
- `WorkboardDispatchPump` 每分钟调用官方 `workboard dispatch`，负责提升依赖、发现超时/失联并启动 ready 卡；它不占用任何角色主会话。
- `WorkboardNotificationPump` 每两分钟读取可重放终态事件并推进 cursor，经賈南風 Telegram 主动汇报；无事件使用 `NO_REPLY` 静默。
- 条件续办由父子卡依赖和调度泵完成；父卡成功后子卡自动进入 ready 并启动，不要求少主再次说“继续”。
- 任意长任务仍由责任角色的独立 worker 执行；A2A 只用于短消息协作，不再兼任任务状态数据库。
- `housekeeper-workboard-control 1.0.3` 仅向 housekeeper 暴露固定 dispatch 与按 UUID 只读 show；不开放任意 shell、路径、配置、消息或文件写入。
- 本次变更不改变八角色人格、Telegram binding、私人 transcript、长期记忆、既有 life 自动化或 ops 凭据入口。

完整部署与验证事实见 `WorkboardTaskControl工作板任务控制部署报告-v0.01.md`。

## 11. Codex 专属任务面板增量

少主不在电脑旁时，可在已认证 Telegram 会话中明确把任务交给 Codex。賈南風按既有大总管职责整理目标与范围，在官方 Workboard 独立 `codex` board 登记；她只登记、查询、取消、催办和汇总，不亲自执行，也不占住 Telegram 主会话。

`codex` 与 `production` 是隔离队列：现有 `WorkboardDispatchPump` 仍固定只派发 `production`，不得把 Codex 卡交给普通 OpenClaw worker。Windows 上的 Codex Desktop standalone Scheduled 每小时触发一个独立任务；空队列只做轻量 discovery 并静默结束，发现合规 `ready` 卡时最多用官方 claim 领取一张，并由本次独立 Scheduled 直接执行，不嵌套创建第二层 Codex task。

每张卡固定执行：项目设计与现场读取、OpenClaw 官方/GitHub/社群检索、正式计划、三个不同关注面的独立审核、按计划实施、真实验收、文档与 GitHub 同步、賈南風 Telegram 最终通知。三审前不产生生产副作用，高风险和缺少必要输入进入 blocked，不把固定流程当成永久高风险授权。

Workboard 是用户可见任务状态，Codex standalone Scheduled run 是执行事实，本机只保存 card/automation、claim token 和阶段映射。领取使用互斥、官方原子 claim 和幂等键；执行失败不无限重做副作用，通知失败只补发通知。

本机 SSH 凭据只以当前 Windows 用户 DPAPI 密文保存于仓库外，不进入卡片、提示词、Git、日志、transcript 或记忆。由于任务访问本地项目，Windows 电脑必须开机且 Codex Desktop 保持运行；离线期间不得声称已经按小时扫描，恢复后由下一周期继续。
