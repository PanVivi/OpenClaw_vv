# 001 OpenClaw架设部署｜QuickBrief 快速简报｜v1.11
...
- 少主在賈南風 Telegram 明确说任务交给 Codex，賈南風登记到独立 `codex` Workboard；她不亲自执行，也不阻塞主会话。
- Codex Desktop 每小时启动一次 standalone 独立任务；空队列轻扫后静默，合规卡由本次 Scheduled 用官方 claim 领取并直接执行。现有 `production` dispatcher 不派发 `codex`。
- 固定质量门：联网资料与现场证据、正式计划、三轮不同审核、实施、真实验收、文档/GitHub 同步、賈南風 Telegram 最终通知。
- 本机凭据使用仓库外 DPAPI；电脑需开机且 Codex Desktop 运行，离线时不会伪装为已扫描。
