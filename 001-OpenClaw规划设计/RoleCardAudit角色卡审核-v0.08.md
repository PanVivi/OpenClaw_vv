# 001 OpenClaw架设部署｜RoleCardAudit 角色卡复审｜v0.08

## v0.08 新一轮结论

本轮逐套核对人格、职责、工具、权限、共同协议和历史快照：

- 八套当前角色卡没有普遍缩减；人格和主体职责保留。
- 真正回归项是：统一风险分级未贯穿 ops/reviewer、非阻塞只部署到 housekeeper、ops/coder 原设计子 Agent被降为未启用、五套 AGENTS 共同协议版本滞后。
- 已升版并修复：賈南風 v1.11、魚玄機 v0.13、步非煙 v0.08、夏姬 v0.06、蕭觀音 v0.08、三 companion v0.05、共同协议 v0.05。
- 八角色均仅获同一 Agent ID 的隔离子 Agent；工具仍受父角色上限。companion 不获工程权限，reviewer 子 Agent不作最终门控，life 子 Agent不替代持久自动化。
- `sessions_history` 继续拒绝，个人记忆只能来自本 Agent transcript；这项已验证修正不得因“恢复原设计”而倒退。
- 所有身份、人设、称呼、companion 直达和 life/housekeeper 路由边界均未修改。

审核结论：设计可部署；生产通过以配置 validate、八角色 spawn Smoke Test、Telegram probe、A2A/记忆/路由不回归为条件。
