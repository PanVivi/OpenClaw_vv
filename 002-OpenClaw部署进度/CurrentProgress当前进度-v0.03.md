...
- 魚玄機与步非煙已完成联动终审并升级为 v0.03 `CANDIDATE`；各自 v0.01、v0.02 均为 `REJECTED`、不可部署或作为后续底稿。
- reviewer、獨孤伽羅、武曌、呂雉当前设计仍为 v0.01 `CANDIDATE`。
- 共同协议 v0.02 的执行摘要已内嵌到八个当前角色的 `AGENTS.md`。

## 魚玄機与步非煙 v0.03 修正

- 魚玄機补齐授权真实性、统一生命周期、取消撤权、防重复、状态与证据持久化、依赖不可用降级、会话硬白名单、外部依赖控制、并发基线、权限回收、证据脱敏和 reviewer.Test 五次失败熔断。
- 步非煙补齐固定 Git 基线、隔离 workspace、路径与网络边界、并发保护、依赖完整性、取消撤权、依赖不可用降级、交付持久化、A2A 硬白名单、证据脱敏、返工和五次熔断。
...
- 两张 `CANDIDATE-候选版.md` 已同步为 v0.03，消除旧 v0.01 标识造成的部署误导。
...
- 賈南風 v1.04、蕭觀音 v0.02、魚玄機 v0.03、步非煙 v0.03 及其他候选角色均未完成对应运行时部署验收。
- 魚玄機的精确生产权限、回收机制、持久化、取消撤权、依赖降级、A2A、两条工程轨、熔断和真实 Smoke Test 待验证。
- 步非煙的隔离 workspace、路径 allow/deny、生产网络/数据拒绝、依赖完整性、取消撤权、依赖降级、交付持久化和返工流程待验证。
...
3. 分 Change 部署和验收魚玄機 v0.03 与步非煙 v0.03，先验证权限拒绝，再验证允许能力、恢复、降级、熔断和联动流程。
