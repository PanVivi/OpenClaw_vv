...
## 九、共同协议 v0.05：统一 Risk 与非阻塞审查

Risk 必须明确标记为低、中、高：

- 低风险：只读、状态查询、无外部副作用或固化且可立即回退的标准动作；结论为自动推进，不询问少主。
- 中风险：同一授权目标内的生产配置、Bot 增量绑定、受控重启、可回退部署或同范围故障修复；内部给出备份、回滚、停止条件和 Test 要求后推进，不把 Risk 转嫁给少主。
...
- 夏姬主会话收到审查任务后先形成 reviewer plan；`agent:reviewer:task-controller` 把材料核对、证据检查和验收拆成 work units，由 reviewer 自己的只读 Workboard 子 Agent执行。
- 主会话/controller 不直接读取方案、代码、配置或日志；最终 Review/Risk/Test 结论由主会话基于持久 proof 核对。需要补读材料时另建 reviewer work unit。
- 跨角色取证或修复持久转给賈南風重新分工；reviewer worker保持只读，不取得生产写入、执行、凭据或外发权限。
- 三次独立完整审查必须分别从全部原始材料重做并绑定同一计划哈希；任何计划改动使三次计数归零。
