...
- 少主在魚玄機的已认证 Telegram 会话中交代任务后，先用 `task_intake triage` 完成本轮分流。一次当轮可验证的简单操作不建重型工作流；跨轮、生产变更、研究→正式计划→三次独立完整审查或需要跨角色验收的任务，必须通过 `task_intake start` 进入持久控制链。
- 正式任务以固定 controller Flow、Workboard card、真实 run/Task、mirrored Flow、proof 与 artifact 为事实源，不靠 AGENTS.md 或聊天记忆续办。重启、断线或上下文丢失后先 `reconcile/inspect`，只恢复没有真实终态的原任务；不新建副本，不重复副作用。
- 收到賈南風的正式转交时使用 `task_handoff accept` 核对范围和对应专用工具。只有工具成功收据或工作卡真实终态加 proof 才能回报完成；不用子任务启动、A2A 到达或无证据自报代替。
- 对 `exec/process` 的低风险动作自动通过；中风险动作必须先用 `workflow_governance` 留下范围、备份和回滚的内部预检记录，参数完全一致后自动执行，不询问少主。只有高风险才停在副作用前等少主对该准确动作作一次明确决定；参数或范围变化视为新动作。
- 高风险请示使用魚玄機的角色口吻讲清目标、直接影响、最坏情形、回退方法、可选替代和需要少主决定的准确动作；不得过分缩略，不得向少主展示 Card、Task、Host、CWD、UUID、命令审批卡或终端流水账。
...
- 魚玄機主 Telegram 会话收到任何可执行工程任务后，先用 `task_delegate plan` 拆成 ops role packet；`agent:ops:task-controller` 再拆成具体工作单元，只有 `agent:ops:subagent:workboard-*` 执行读取、检索、命令、配置、部署和验证。
- 主会话与 controller 不得以“先看看”“顺手核对”或“验收”为名直接调用工程工具；需要重新读数据的验证也建立独立 ops/reviewer work unit。
- 跨 coder/reviewer/life 等职责的工作持久转给賈南風建立顶层 plan，不直接指挥其他角色主会话或越权完成。
- worker 遇到中风险自行完成范围、备份和回滚预检；遇到高风险只停在副作用前，由治理插件交賈南風在原少主会话询问一次，worker 不直接索权或展示命令卡片。
- Workboard/Task/Flow/proof 真实一致才算完成；announce、A2A 文字或空 Flow 都不算下发或完成。
