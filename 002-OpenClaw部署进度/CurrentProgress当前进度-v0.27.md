# 002 OpenClaw架设部署｜CurrentProgress 当前进度｜v0.27

核验时间：2026-07-28 21:55 +08:00
分支：`codex/fix-life-memo-permission`

## v0.27 萧观音少主专属资料区

- 回溯 life 真实会话与 Workboard 记录后确认：原 `workspaceAccess=rw` 没有对应文件工具，通用文件与 shell 能力又被 deny，故“无法创建备忘录”是有效权限缺口。
- 用户确认不采用只服务备忘录的窄工具，改为 life workspace 内固定 `users/Vivi/` 专属生活资料区。
- 生产已安装 `life-memo` 1.1.0，仅向 life 注册 `life_files`，支持受限 `list/get/mkdir/create/update/append`；不开放删除、移动、shell 或任意工程写入。
- life 角色卡 v0.12 五件套已从固定提交 `7eaaaa2` 原子同步；通用 deny、`life_automation`、Telegram binding、session 和 memory 均保留。
- 首次真实创建与 v0.12 部署后回读均通过：工具调用 4 次、0 失败，LongCat-2.0 无 fallback。
- ops 真实系统提示不含 `life_files`；文件 `0600`、目录 `0700`；部署前 160 个 life session 文件无一丢失。
- OpenClaw 配置 valid、plugin doctor 无问题、Gateway probe ok、八个 Telegram account probe ok。
- 完整证据见 `LifeOwnerFiles萧观音少主专属资料区修复与验收报告-v0.01.md`。
